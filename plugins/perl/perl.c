    ...
    while (fields[field_index] != NULL)
      {
          ...
          switch (fields[field_index][0])
            ...
            case 't':
                field_value =
                    newSVnv (xchat_list_time (ph, list, field_name));
                break;
            default:
                field_value = &PL_sv_undef;
            }
...
array2av (char *array[])
{
    ...
    for (count = 1;
         count < 32 && array[count] != NULL && array[count][0] != 0; count++)
      {
...
XS (XS_Xchat_send_modes)
{
    ...
    if (items < 3 || items > 4)
    ...
    else
      {
          ...
          xchat_send_modes (ph, targets, target_count, modes_per_line, sign,
                            mode);
          free (targets);
      }
}
    ...
    else
      {
          ...
          if (!strncmp ("win_ptr", SvPV_nolen (id), 7))
          ...
          else
            {
                ...
                if (!strncmp ("libdirfs", SvPV_nolen (id), 8) ||
...
XS (XS_Xchat_hook_command)
{
    ...
    if (items != 5)
    ...
    else
      {
          ...
          data->package = NULL;
          hook =
              xchat_hook_command (ph, name, pri, command_cb, help_text, data);
                   ...
                   char **plugin_desc, char **plugin_version, char *arg)
{
    ...
    *plugin_version = PACKAGE_VERSION;
    ...
    xchat_hook_command (ph, "reload", XCHAT_PRI_NORM, perl_command_reload, 0,
                        0);
    xchat_hook_command (ph, "pl_reload", XCHAT_PRI_NORM, perl_command_reload,
                        0, 0);
