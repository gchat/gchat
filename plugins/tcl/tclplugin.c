...
static char RCSID[] =
    "$Id: tclplugin.c,v 1.64 2010/03/10 04:24:16 mooooooo Exp $";
...
static void
Tcl_MyDStringAppend (Tcl_DString * ds, char *string)
{
    Tcl_DStringAppend (ds, string, strlen (string));
}

static char *
InternalProcName (int value)
{
...
static int
SourceInternalProc (int id, const char *args, const char *source)
{
    Tcl_DString ds;
    int result;
    Tcl_DStringInit (&ds);
...
static int
EvalInternalProc (const char *procname, int ct, ...)
{
    Tcl_DString ds;
    int result;
    va_list ap;
...
static void
DeleteInternalProc (const char *proc)
{
    Tcl_DString ds;
...
static char *
StrDup (const char *string, int *length)
{
    char *result;
...
static xchat_context *
xchat_smart_context (const char *arg1, const char *arg2)
{
                ...
                while (xchat_list_next (ph, list))
                  {
                      ...
                      ctx =
                          (xchat_context *) xchat_list_str (ph, list,
                                                            "context");
                      ...
                      if (actx)
                      ...
                      else
                        {
                            ...
                            if (xchat_list_int (ph, list, "type") == 1)
                              ...
                              {
                                  if ((strcasecmp (server, s) == 0)
                                      &&
                                      (strcasecmp
                                       (arg1,
                                        xchat_list_str (ph, list,
                                                        "channel")) == 0))
                                    {
                                        result = ctx;
                                        break;
...
static int
insert_timer (int seconds, int count, const char *script)
{
    int x;
    int dummy;
    time_t now;
          ...
          if (timers[x].timerid == 0)
            {
                if (SourceInternalProc (id, "", script) == TCL_ERROR)
                  {
                      xchat_printf (ph,
                                    "\0039TCL plugin\003\tERROR (timer %d) ",
                                    timers[x].timerid);
                      NiceErrorInfo ();
                      return (-1);
                  }
...
do_timer ()
{
    ...
    if (EvalInternalProc (timers[index].procPtr, 0) == TCL_ERROR)
      {
          xchat_printf (ph, "\0039TCL plugin\003\tERROR (timer %d) ",
...
static int
Server_raw_line (char *word[], char *word_eol[], void *userdata)
{
    ...
    if (entry != NULL)
      {
          ...
          if (Tcl_SplitList (interp, procList, &list_argc, &list_argv) ==
              TCL_OK)
            {
                ...
                for (count = 0; count < list_argc; count++)
                  {
                      ...
                      if (Tcl_SplitList
                          (interp, list_argv[count], &proc_argc,
                           &proc_argv) != TCL_OK)
                          continue;

                      origctx = xchat_get_context (ph);
                      if (EvalInternalProc
                          (proc_argv[1], 7, src, dest, cmd, rest, word_eol[1],
                           proc_argv[0], myitoa (private)) == TCL_ERROR)
                        {
                            xchat_printf (ph,
                                          "\0039TCL plugin\003\tERROR (on %s %s) ",
                                          cmd, proc_argv[0]);
                            NiceErrorInfo ();
                        }
                      xchat_set_context (ph, origctx);
                      ...
                      if ((complete[complete_level].result ==
                           XCHAT_EAT_PLUGIN)
                          || (complete[complete_level].result ==
                              XCHAT_EAT_ALL))
                          break;
...
static int
Print_Hook (char *word[], void *userdata)
{
    char *procList;
    Tcl_HashEntry *entry;
    xchat_context *origctx;
    ...
    if ((entry =
         Tcl_FindHashEntry (&cmdTablePtr, xc[(int) userdata].event)) != NULL)
      {
                ...
                for (count = 0; count < list_argc; count++)
                  {
                      ...
                      if ((int) userdata == CHAT)
                        {
                            Tcl_DStringAppend (&ds, word[3],
                                               strlen (word[3]));
                            Tcl_DStringAppend (&ds, "!*@", 3);
                            Tcl_DStringAppend (&ds, word[1],
                                               strlen (word[1]));
                            if (EvalInternalProc
                                (proc_argv[1], 7, ds.string, word[2],
                                 xc[(int) userdata].event, word[4], "",
                                 proc_argv[0], "0") == TCL_ERROR)
                              {
                                  xchat_printf (ph,
                                                "\0039TCL plugin\003\tERROR (on %s %s) ",
                                                xc[(int) userdata].event,
                                                proc_argv[0]);
                                  NiceErrorInfo ();
                              }
                        }
                      else
                        {
                            if (xc[(int) userdata].argc > 0)
                              {
                                  for (x = 0; x <= xc[(int) userdata].argc;
                                       x++)
                                      Tcl_DStringAppendElement (&ds, word[x]);
                              }
                            if (EvalInternalProc
                                (proc_argv[1], 7, "", "",
                                 xc[(int) userdata].event, "", ds.string,
                                 proc_argv[0], "0") == TCL_ERROR)
                              {
                                  xchat_printf (ph,
                                                "\0039Tcl plugin\003\tERROR (on %s %s) ",
...
static int
tcl_timerexists (ClientData cd, Tcl_Interp * irp, int argc,
                 const char *argv[])
{
    int x;
    int timerid;
...
static int
tcl_killtimer (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    int x;
    int timerid;
...
static int
tcl_timers (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    int x;
    Tcl_DString ds;
    time_t now;
          ...
          if (timers[x].timerid)
            {
                ...
                Tcl_DStringAppendElement (&ds,
                                          myitoa ((long) timers[x].timerid));
                Tcl_DStringAppendElement (&ds,
                                          myitoa ((long) timers[x].timestamp -
                                                  now));
                Tcl_DStringAppendElement (&ds, timers[x].procPtr);
                Tcl_DStringAppendElement (&ds,
                                          myitoa ((long) timers[x].seconds));
                Tcl_DStringAppendElement (&ds,
                                          myitoa ((long) timers[x].count));
...
static int
tcl_timer (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    int seconds;
    int timerid;
    int repeat = 0;
    ...
    BADARGS (3, 6,
             " ?-repeat? ?-count times? seconds {script | procname ?args?}");
...
static int
tcl_on (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    int newentry;
    char *procList;
    Tcl_HashEntry *entry;
    ...
    if (SourceInternalProc
        (id, "_src _dest _cmd _rest _raw _label _private",
         argv[3]) == TCL_ERROR)
      {
          xchat_printf (ph, "\0039Tcl plugin\003\tERROR (on %s:%s) ", argv[1],
                        argv[2]);
          NiceErrorInfo ();
          return TCL_OK;
      }
                ...
                if (strcmp (xc[index].event, token) == 0)
                  {
                      if (xc[index].hook == NULL)
                        {
                            xc[index].hook =
                                xchat_hook_print (ph, xc[index].emit,
                                                  XCHAT_PRI_NORM, Print_Hook,
                                                  (void *) index);
                            break;
                        }
                  }
...
static int
tcl_off (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    char *procList;
    Tcl_HashEntry *entry;
    char *token;
...
static int
tcl_alias (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    int newentry;
    alias *aliasPtr;
    Tcl_HashEntry *entry;
    ...
    if (strlen (argv[argc - 1]))
      {
          ...
          if (SourceInternalProc (id, "_cmd _rest", argv[argc - 1]) ==
              TCL_ERROR)
            {
                xchat_printf (ph, "\0039Tcl plugin\003\tERROR (alias %s) ",
                              argv[1]);
                NiceErrorInfo ();
                return TCL_OK;
            }
          ...
          if (newentry)
            {
                ...
                if (string[0] == '@')
                    ...
                    aliasPtr->hook =
                        xchat_hook_command (ph, string, XCHAT_PRI_NORM,
                                            Command_Alias, help, 0);
            }
          else
            {
...
static int
tcl_complete (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    BADARGS (1, 2, " ?EAT_NONE|EAT_XCHAT|EAT_PLUGIN|EAT_ALL?");

    if (argc == 2)
      {
          if (Tcl_GetInt (irp, argv[1], &complete[complete_level].result) !=
              TCL_OK)
            {
                if (strcasecmp ("EAT_NONE", argv[1]) == 0)
                    ...
                    BADARGS (1, 2,
                             " ?EAT_NONE|EAT_XCHAT|EAT_PLUGIN|EAT_ALL?");
            }
      }
    else
...
static int
tcl_command (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    xchat_context *origctx;
    xchat_context *ctx = NULL;
...
static int
tcl_raw (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    xchat_context *origctx;
    xchat_context *ctx = NULL;
    const char *string = NULL;
...
static int
tcl_prefs (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    int i;
    const char *str;
...
static int
tcl_info (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[],
          char *id)
{
    char *result;
    int max_argc;
...
static int
tcl_me (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    return tcl_info (cd, irp, argc, argv, "nick");
}

static int
tcl_getinfo (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    return tcl_info (cd, irp, argc, argv, NULL);
}

static int
tcl_getlist (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
...
static int
tcl_xchat_puts (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    Tcl_Channel chan;
    const char *string;
    int newline;
    ...
    switch (argc)
      ...
      default:
          Tcl_AppendResult (interp, argv, "?-nonewline? ?channelId? string",
                            NULL);
          return TCL_ERROR;
      }
...
static int
tcl_print (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    xchat_context *origctx;
    xchat_context *ctx = NULL;
    const char *string = NULL;
...
static int
tcl_setcontext (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    xchat_context *ctx = NULL;
...
static int
tcl_findcontext (ClientData cd, Tcl_Interp * irp, int argc,
                 const char *argv[])
{
    xchat_context *ctx = NULL;
...
static int
tcl_getcontext (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    xchat_context *ctx = NULL;
...
static int
tcl_channels (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
          ...
          while (xchat_list_next (ph, list))
            {
                if (xchat_list_int (ph, list, "type") != 2)
                    continue;
                if (strcasecmp (server, xchat_list_str (ph, list, "server"))
                    != 0)
                    continue;
...
static int
tcl_servers (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    const char *server;
    xchat_list *list;
    Tcl_DString ds;
...
static int
tcl_queries (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    const char *server, *channel;
    xchat_list *list;
    Tcl_DString ds;
...
static int
tcl_users (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
          ...
          while (xchat_list_next (ph, list))
            {
                ...
                Tcl_DStringAppendElement (&ds,
                                          myitoa ((long)
                                                  xchat_list_int (ph, list,
                                                                  "away")));
                Tcl_DStringAppendElement (&ds,
                                          myitoa ((long)
                                                  xchat_list_time (ph, list,
                                                                   "lasttalk")));
                Tcl_DStringAppendElement (&ds,
                                          myitoa ((long)
                                                  xchat_list_int (ph, list,
...
static int
tcl_notifylist (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    xchat_list *list;
    Tcl_DString ds;
...
static int
tcl_chats (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    xchat_list *list;
    Tcl_DString ds;
...
static int
tcl_ignores (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    xchat_list *list;
    Tcl_DString ds;
    int flags;
...
static int
tcl_dcclist (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    xchat_list *list;
    Tcl_DString ds;
    int dcctype;
...
static int
tcl_strip (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    char *new_text;
    int flags = 1 | 2;
...
static int
tcl_topic (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    xchat_context *origctx, *ctx = NULL;
    BADARGS (1, 3, " ?server|network|context? ?channel?");
...
static int
tcl_xchat_nickcmp (ClientData cd, Tcl_Interp * irp, int argc,
                   const char *argv[])
{
    BADARGS (3, 3, " string1 string2");
    Tcl_AppendResult (irp,
                      myitoa ((long) xchat_nickcmp (ph, argv[1], argv[2])),
                      NULL);
    return TCL_OK;
}

static int
tcl_word (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    int index;
...
static int
tcl_word_eol (ClientData cd, Tcl_Interp * irp, int argc, const char *argv[])
{
    int index;
    ...
    if (!index || (index > 31))
        ...
        Tcl_AppendResult (interp, complete[complete_level].word_eol[index],
                          NULL);
...
static int
Command_Alias (char *word[], char *word_eol[], void *userdata)
{
    alias *aliasPtr;
    Tcl_HashEntry *entry;
    xchat_context *origctx;
    ...
    if ((entry = Tcl_FindHashEntry (&aliasTablePtr, string)) != NULL)
      {
          aliasPtr = Tcl_GetHashValue (entry);
          origctx = xchat_get_context (ph);
          if (EvalInternalProc (aliasPtr->procPtr, 2, string, word_eol[2]) ==
              TCL_ERROR)
            {
                xchat_printf (ph, "\0039Tcl plugin\003\tERROR (alias %s) ",
                              string);
...
static int
Null_Command_Alias (char *word[], char *word_eol[], void *userdata)
{
    alias *aliasPtr;
    Tcl_HashEntry *entry;
    xchat_context *origctx;
    ...
    if ((entry = Tcl_FindHashEntry (&aliasTablePtr, string)) != NULL)
      {
          aliasPtr = Tcl_GetHashValue (entry);
          origctx = xchat_get_context (ph);
          if (EvalInternalProc (aliasPtr->procPtr, 2, string, word_eol[1]) ==
              TCL_ERROR)
            {
                xchat_printf (ph, "\0039Tcl plugin\003\tERROR (alias %s) ",
                              string);
...
static int
Command_TCL (char *word[], char *word_eol[], void *userdata)
{
...
static int
Command_Source (char *word[], char *word_eol[], void *userdata)
{
    const char *xchatdir;
    Tcl_DString ds;
    struct stat dummy;
                ...
                if (!strchr (word_eol[2], '/'))
                  {
                      ...
                      Tcl_DStringAppend (&ds, "/", 1);
                      Tcl_DStringAppend (&ds, word_eol[2],
                                         strlen (word_eol[2]));
                  }
            }
...
static int
Command_Reloadall (char *word[], char *word_eol[], void *userdata)
{
    Tcl_Plugin_DeInit ();
    Tcl_Plugin_Init ();
...
static int
TCL_Event_Handler (void *userdata)
{
                   ...
                   char **plugin_desc, char **plugin_version, char *arg)
{
    ...
    *plugin_version = VERSION;
    ...
    raw_line_hook =
        xchat_hook_server (ph, "RAW LINE", XCHAT_PRI_NORM, Server_raw_line,
                           NULL);
    Command_TCL_hook =
        xchat_hook_command (ph, "tcl", XCHAT_PRI_NORM, Command_TCL, 0, 0);
    Command_Source_hook =
        xchat_hook_command (ph, "source", XCHAT_PRI_NORM, Command_Source, 0,
                            0);
    Command_Reload_hook =
        xchat_hook_command (ph, "reloadall", XCHAT_PRI_NORM,
                            Command_Reloadall, 0, 0);
    Command_Load_hook =
        xchat_hook_command (ph, "LOAD", XCHAT_PRI_NORM, Command_Source, 0, 0);
    Event_Handler_hook = xchat_hook_timer (ph, 100, TCL_Event_Handler, 0);
    Null_Command_hook =
        xchat_hook_command (ph, "", XCHAT_PRI_NORM, Null_Command_Alias, "",
                            0);
