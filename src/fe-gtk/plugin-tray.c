...
static gboolean
libnotify_notify_new (const char *title, const char *text,
                      GtkStatusIcon * icon)
{
    void *noti;
...
tray_menu_cb (GtkWidget * widget, guint button, guint time, gpointer userdata)
{
    ...
    tray_make_item (menu, NULL, tray_menu_quit_cb, NULL);
    mg_create_icon_item (_("_Quit"), GTK_STOCK_QUIT, menu, tray_menu_quit_cb,
                         NULL);
    ...
    gtk_menu_popup (GTK_MENU (menu), NULL, NULL,
                    gtk_status_icon_position_menu, userdata, button, time);
}
...
tray_hilight_cb (char *word[], void *userdata)
{
    ...
    if (prefs.input_balloon_hilight)
        tray_set_balloonf (word[2],
                           _("GChat: Highlighted message from: %s (%s)"),
                           word[1], xchat_get_info (ph, "channel"));
...
tray_message_cb (char *word[], void *userdata)
{
    ...
    if (prefs.input_balloon_chans)
        tray_set_balloonf (word[2],
                           _("GChat: New public message from: %s (%s)"),
                           word[1], xchat_get_info (ph, "channel"));
...
tray_priv (char *from, char *text)
{
    ...
    if (tray_priv_count == 1)
        tray_set_tipf (_("GChat: Pvt from: %s (%s)"), from, network);
    else
        tray_set_tipf (_("GChat: %u pvts, latest from: %s (%s)"),
                       tray_priv_count, from, network);

    if (prefs.input_balloon_priv)
        tray_set_balloonf (text, _("GChat: Pvt from: %s (%s)"),
                           from, network);
}
                  ...
                  char **plugin_desc, char **plugin_version, char *arg)
{
    ...
    *plugin_version = "";
    ...
    xchat_hook_print (ph, "Channel Action Hilight", -1, tray_hilight_cb,
                      NULL);
    ...
    xchat_hook_print (ph, "Private Message to Dialog", -1, tray_priv_cb,
                      NULL);
