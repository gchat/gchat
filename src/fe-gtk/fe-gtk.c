...
root_event_cb (GdkXEvent * xev, GdkEventProperty * event, gpointer data)
{
    ...
    if (xevent->type == PropertyNotify)
      {
          if (at == None)
              at = XInternAtom (xevent->xproperty.display, "_XROOTPMAP_ID",
                                True);
...
static const GOptionEntry gopt_entries[] = {
    ...
    {"url", 0, 0, G_OPTION_ARG_STRING, &arg_url,
...
const char cursor_color_rc[] = "style \"xc-ib-st\"" "{"
#ifdef USE_GTKSPELL
...
fe_set_topic (session * sess, char *topic, char *stripped_topic)
{
    if (!sess->gui->is_tab || sess == current_tab)
      {
          gtk_entry_set_text (GTK_ENTRY (sess->gui->topic_entry),
                              stripped_topic);
...
static void
fe_update_mode_entry (session * sess, GtkWidget * entry, char **text,
                      char *new_text)
{
    if (!sess->gui->is_tab || sess == current_tab)
      {
...
{
    PrintTextRaw (sess->res->buffer, (unsigned char *) text,
                  prefs.indent_nicks, stamp);
                ...
                if (!sess->gui->is_tab || current_tab == sess)
                  {
                      ...
                      if (sess->gui->laginfo)
                          gtk_label_set_text ((GtkLabel *) sess->gui->laginfo,
                                              lagtext);
                  }
                else
                  {
    ...
    while (list)
      {
          ...
          if (sess->server == serv)
            {
                snprintf (tbuf, sizeof (tbuf) - 1, _("%d bytes"),
                          serv->sendq_len);
                snprintf (tip, sizeof (tip) - 1,
                          _("Network send queue: %d bytes"), serv->sendq_len);
                ...
                if (!sess->gui->is_tab || current_tab == sess)
                  {
                      ...
                      if (sess->gui->throttleinfo)
                          gtk_label_set_text ((GtkLabel *) sess->gui->
                                              throttleinfo, tbuf);
                  }
                else
                  {
    ...
    while (list)
      {
          sess = list->data;
          if (sess->server == serv
              && (current_tab == sess || !sess->gui->is_tab))
            {
                ...
                switch (type)
                  ...
                  case FE_SE_RECONDELAY:       /* reconnect delay begun */
                      ...
                      gtk_widget_set_sensitive (gui->
                                                menu_item[MENU_ID_DISCONNECT],
                                                1);
                      break;
             ...
             int flags)
{
    ...
    gtkutil_file_req (title, callback, userdata, initial,
