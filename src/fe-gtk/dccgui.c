...
static void
dcc_prepare_row_chat (struct DCC *dcc, GtkListStore * store,
                      GtkTreeIter * iter, gboolean update_only)
{
    ...
    gtk_list_store_set (store, iter,
                        ...
                        NULL : colors + dccstat[dcc->dccstat].color, -1);
}

static void
dcc_prepare_row_send (struct DCC *dcc, GtkListStore * store,
                      GtkTreeIter * iter, gboolean update_only)
{
...
static void
dcc_prepare_row_recv (struct DCC *dcc, GtkListStore * store,
                      GtkTreeIter * iter, gboolean update_only)
{
...
static gboolean
dcc_find_row (struct DCC *find_dcc, GtkTreeModel * model, GtkTreeIter * iter,
              int col)
{
    struct DCC *dcc;
...
static GSList *
treeview_get_selected (GtkTreeModel * model, GtkTreeSelection * sel,
                       int column)
{
    GtkTreeIter iter;
    GSList *list = NULL;
...
static void
dcc_add_column (GtkWidget * tree, int textcol, int colorcol, char *title,
...
fe_dcc_open_recv_win (int passive)
{
    ...
    gtk_tree_view_column_set_expand (gtk_tree_view_get_column
                                     (GTK_TREE_VIEW (view), COL_FILE), TRUE);
    gtk_tree_view_column_set_expand (gtk_tree_view_get_column
                                     (GTK_TREE_VIEW (view), COL_NICK), TRUE);
    ...
    gtk_table_attach (GTK_TABLE (table), radio, 3, 4, 0, 1, GTK_FILL,
                      GTK_FILL, 0, 0);
    ...
    gtk_table_attach (GTK_TABLE (table), radio, 1, 2, 0, 1, GTK_FILL,
                      GTK_FILL, 0, 0);
    group = gtk_radio_button_get_group (GTK_RADIO_BUTTON (radio));

    radio = gtk_radio_button_new_with_mnemonic (group, _("Downloads"));
    g_signal_connect (G_OBJECT (radio), "toggled",
                      G_CALLBACK (dcc_toggle),
                      GINT_TO_POINTER (VIEW_DOWNLOAD));
    gtk_table_attach (GTK_TABLE (table), radio, 2, 3, 0, 1, GTK_FILL,
                      GTK_FILL, 0, 0);

    exp = gtk_expander_new (_("Details"));
    gtk_table_attach (GTK_TABLE (table), exp, 0, 1, 0, 1,
    ...
    gtk_table_attach (GTK_TABLE (table), detailbox, 0, 4, 1, 2,
                      GTK_EXPAND | GTK_FILL, GTK_FILL, 0, 0);
    ...
    dccfwin.abort_button =
        gtkutil_button (bbox, GTK_STOCK_CANCEL, 0, abort_clicked, 0,
                        _("Abort"));
    dccfwin.accept_button =
        gtkutil_button (bbox, GTK_STOCK_APPLY, 0, accept_clicked, 0,
                        _("Accept"));
    dccfwin.resume_button =
        gtkutil_button (bbox, GTK_STOCK_REFRESH, 0, resume_clicked, 0,
                        _("Resume"));
    dccfwin.open_button =
        gtkutil_button (bbox, 0, 0, browse_dcc_folder, 0,
                        _("Open Folder..."));
    gtk_widget_set_sensitive (dccfwin.accept_button, FALSE);
    gtk_widget_set_sensitive (dccfwin.resume_button, FALSE);
    gtk_widget_set_sensitive (dccfwin.abort_button, FALSE);
...
fe_dcc_open_chat_win (int passive)
{
    ...
    dcccwin.abort_button =
        gtkutil_button (bbox, GTK_STOCK_CANCEL, 0, abort_chat_clicked, 0,
                        _("Abort"));
    dcccwin.accept_button =
        gtkutil_button (bbox, GTK_STOCK_APPLY, 0, accept_chat_clicked, 0,
                        _("Accept"));
    gtk_widget_set_sensitive (dcccwin.accept_button, FALSE);
    gtk_widget_set_sensitive (dcccwin.abort_button, FALSE);
