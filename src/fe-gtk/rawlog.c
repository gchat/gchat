...
open_rawlog (struct server *serv)
{
    ...
    serv->gui->rawlog_window =
        mg_create_generic_tab ("RawLog", tbuf, FALSE, TRUE, close_rawlog,
                               serv, 640, 320, &vbox, serv);
    ...
    gtk_xtext_set_tint (GTK_XTEXT (serv->gui->rawlog_textlist),
                        prefs.tint_red, prefs.tint_green, prefs.tint_blue);
    ...
    gtk_xtext_set_font (GTK_XTEXT (serv->gui->rawlog_textlist),
                        prefs.font_normal);
    GTK_XTEXT (serv->gui->rawlog_textlist)->ignore_hidden = 1;
    gtk_widget_show (serv->gui->rawlog_textlist);

    vscrollbar =
        gtk_vscrollbar_new (GTK_XTEXT (serv->gui->rawlog_textlist)->adj);
