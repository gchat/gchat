 ...
 */
static void
cv_tabs_sizerequest (GtkWidget * viewport, GtkRequisition * requisition,
                     chanview * cv)
{
    if (!cv->vertical)
        requisition->width = 1;
...
static void
cv_tabs_sizealloc (GtkWidget * widget, GtkAllocation * allocation,
                   chanview * cv)
{
    GtkAdjustment *adj;
    GtkWidget *inner;
    ...
    if (cv->vertical)
      {
          ...
          gdk_window_get_geometry (inner->parent->window, 0, 0, 0,
                                   &viewport_size, 0);
      }
    else
      {
          adj = gtk_viewport_get_hadjustment (GTK_VIEWPORT (inner->parent));
          gdk_window_get_geometry (inner->parent->window, 0, 0,
                                   &viewport_size, 0, 0);
      }
    ...
    if (!tab_left_is_moving)
      {
          ...
          for (i = adj->value; ((i > new_value) && (tab_left_is_moving));
               i -= 0.1)
            {
    ...
    if (!tab_right_is_moving)
      {
          ...
          for (i = adj->value; ((i < new_value) && (tab_right_is_moving));
               i += 0.1)
            {
                gtk_adjustment_set_value (adj, i);
                while (g_main_pending ())
    ...
    while (boxes)
      {
          ...
          while (children)
            {
                if (!GTK_IS_SEPARATOR
                    (((GtkBoxChild *) children->data)->widget))
                  {
                      empty = FALSE;
                      break;
                  }
...
static gboolean
tab_ignore_cb (GtkWidget * widget, GdkEventCrossing * event,
               gpointer user_data)
{
    return TRUE;
}
...
tab_pressed_cb (GtkToggleButton * tab, chan * ch)
{
    ...
    if (old_tab && old_tab->impl)
      {
          gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (old_tab->impl),
                                        FALSE);
          if (old_tab == ch)
              is_switching = FALSE;
      }
...
tab_group_for_each_tab (chanview * cv,
                        int (*callback) (GtkWidget * tab, int num,
...
cv_tabs_move_family (chan * ch, int delta)
{
    ...
    i = 0;
    for (list = GTK_BOX (((tabview *) ch->cv)->inner)->children; list;
         list = list->next)
      {
          GtkBoxChild *child_entry;
          void *fam;
