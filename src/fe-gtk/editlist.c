...
editlist_gui_save (GtkWidget * igad)
{
    ...
    fh = xchat_open_file (editlist_file, O_TRUNC | O_WRONLY | O_CREAT, 0600,
                          XOF_DOMODE);
    if (fh != -1)
      {
          while (1)
            {
                if (!gtk_clist_get_text
                    (GTK_CLIST (editlist_gui_list), i, 0, &a))
                    break;
...
editlist_gui_movedown (GtkWidget * igad)
{
    ...
    if (row != -1)
      {
          if (!gtk_clist_get_text
              (GTK_CLIST (editlist_gui_list), row + 1, 0, &temp))
              return;
          ...
          if (!gtk_clist_row_is_visible (GTK_CLIST (editlist_gui_list), row)
              != GTK_VISIBILITY_FULL)
              gtk_clist_moveto (GTK_CLIST (editlist_gui_list), row, 0, 0.9,
                                0);
      }
}
    ...
    if (row != -1 && row > 0)
      {
          ...
          if (gtk_clist_row_is_visible (GTK_CLIST (editlist_gui_list), row) !=
              GTK_VISIBILITY_FULL)
              gtk_clist_moveto (GTK_CLIST (editlist_gui_list), row, 0, 0.1,
                                0);
      }
}
                   ...
                   char *wmclass, char *file, char *help)
{
    ...
    button =
        gtkutil_button (hbox, GTK_STOCK_GO_DOWN, 0, editlist_gui_movedown, 0,
                        _("Move Dn"));
    gtk_widget_set_usize (button, 100, 0);
    ...
    button =
        gtkutil_button (hbox, GTK_STOCK_SORT_ASCENDING, 0, editlist_gui_sort,
                        0, _("Sort"));
    gtk_widget_set_usize (button, 100, 0);
