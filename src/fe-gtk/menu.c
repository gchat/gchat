...
GtkWidget *
menu_toggle_item (char *label, GtkWidget * menu, void *callback,
                  void *userdata, int state)
{
    GtkWidget *item;
...
menu_create (GtkWidget * menu, GSList * list, char *target, int check_path)
{
    ...
    while (list)
    {
        ...
        if (!strncasecmp (pop->name, "SUB", 3))
        {
            childcount = 0;
            tempmenu =
                menu_quick_sub (pop->cmd, tempmenu, &subitem,
                                XCMENU_DOLIST | XCMENU_MNEMONIC, -1);

        }
        else if (!strncasecmp (pop->name, "TOGGLE", 6))
        {
            childcount++;
            menu_toggle_item (pop->name + 7, tempmenu, toggle_cb,
                              pop->cmd, cfg_get_bool (pop->cmd));
        ...
        else
        {
                    ...
                    && !strcmp (pop->cmd, "notify -n ASK %s"))
            {
                ...
                if (!target
                        || notify_is_in_list (current_sess->server, target))
                {
                    list = list->next;
                    continue;
                }
            ...
            {
                menu_quick_item (pop->cmd, label, tempmenu, 0, target,
                                 icon);
...
menu_create_nickinfo_menu (struct User *user, GtkWidget * submenu)
{
    ...
    {
        real =
            strip_color (user->realname, -1, STRIP_ALL | STRIP_ESCMARKUP);
    ...
    if (user->away)
    {
        ...
        {
            char *msg =
                strip_color (away->message ? away->message : unknown, -1,
                             STRIP_ALL | STRIP_ESCMARKUP);
...
fe_userlist_update (session * sess, struct User *user)
{
    ...
    g_signal_handlers_disconnect_by_func (nick_submenu, menu_nickinfo_cb,
                                          sess);
               ...
               int num_sel)
{
    ...
    else
    {
        ...
        if (user)
        {
            nick_submenu = submenu =
                               menu_quick_sub (nick, menu, NULL, XCMENU_DOLIST, -1);
    ...
    while (list)
    {
        ...
        if (!sess->gui->is_tab || !maindone)
        {
            if (sess->gui->is_tab)
                maindone = TRUE;
            if (id != -1)
                GTK_CHECK_MENU_ITEM (sess->gui->menu_item[id])->active =
                    state;
            if (callback)
                callback (sess);
        }
...
menu_middlemenu (session * sess, GdkEventButton * event)
{
    ...
    menu =
        menu_create_main (accel_group, FALSE, sess->server->is_away,
                          !sess->gui->is_tab, NULL);
...
menu_urlmenu (GdkEventButton * event, char *url)
{
    ...
    if (strncmp (str_copy, "irc://", 6) == 0 ||
        ...
        menu_quick_item_with_callback (open_url_cb, _("Connect"), menu,
                                       str_copy);
    else
        menu_quick_item_with_callback (open_url_cb, _("Open Link in Browser"),
                                       menu, str_copy);
    menu_quick_item_with_callback (copy_to_clipboard_cb,
                                   _("Copy Selected Link"), menu, str_copy);
...
menu_chanmenu (struct session *sess, GdkEventButton * event, char *chan)
{
    ...
    if (!is_joined)
        menu_quick_item_with_callback (menu_chan_join, _("Join Channel"),
                                       menu, str_copy);
    else
    {
        menu_quick_item_with_callback (menu_chan_part, _("Part Channel"),
                                       menu, str_copy);
        menu_quick_item_with_callback (menu_chan_cycle, _("Cycle Channel"),
                                       menu, str_copy);
    }
...
menu_addfavoritemenu (server * serv, GtkWidget * menu, char *channel)
{
    ...
    if (joinlist_is_in_list (serv, channel))
        mg_create_icon_item (_("_Remove from Favorites"), GTK_STOCK_REMOVE,
                             menu, menu_delfav_cb, serv);
    else
        mg_create_icon_item (_("_Add to Favorites"), GTK_STOCK_ADD, menu,
                             menu_addfav_cb, serv);
}
...
menu_usermenu (void)
{
    editlist_gui_open (NULL, NULL, usermenu_list, _("GChat: User menu"),
...
usermenu_create (GtkWidget * menu)
{
    ...
    menu_quick_item_with_callback (menu_usermenu, _("Edit this menu..."),
                                   menu, 0);
}
...
menu_join (GtkWidget * wid, gpointer none)
{
    ...
    dialog = gtk_dialog_new_with_buttons (_("Join Channel"),
                                          ...
                                          _("Retrieve channel list..."),
                                          GTK_RESPONSE_HELP, GTK_STOCK_CANCEL,
...
#define url_help           _("URL Handlers - Special codes:\n\n"\
                           ...
                           "shell instead of GChat")

static void
menu_usercommands (void)
{
    editlist_gui_open (NULL, NULL, command_list,
                       _("GChat: User defined commands"), "commands",
                       "commands.conf", usercommands_help);
}

static void
menu_ulpopup (void)
{
    editlist_gui_open (NULL, NULL, popup_list,
                       _("GChat: Userlist popup menu"), "popup", "popup.conf",
                       ulbutton_help);
}
...
menu_rpopup (void)
{
    editlist_gui_open (_("Text"), _("Replace with"), replace_list,
                       _("GChat: Auto replace"), "replace", "replace.conf",
                       0);
}
...
{
    editlist_gui_open (NULL, NULL, urlhandler_list, _("GChat: URL Handlers"),
                       "urlhandlers", "urlhandlers.conf", url_help);
}
...
menu_ulbuttons (void)
{
    editlist_gui_open (NULL, NULL, button_list, _("GChat: Userlist buttons"),
                       "buttons", "buttons.conf", ulbutton_help);
}

static void
menu_dlgbuttons (void)
{
    editlist_gui_open (NULL, NULL, dlgbutton_list, _("GChat: Dialog buttons"),
                       "dlgbuttons", "dlgbuttons.conf", dlgbutton_help);
}

static void
menu_ctcpguiopen (void)
{
    editlist_gui_open (NULL, NULL, ctcp_list, _("GChat: CTCP Replies"),
                       "ctcpreply", "ctcpreply.conf", ctcp_help);
}
...
static struct mymenu mymenu[] = {
    ...
    {   N_("Network Li_st..."), menu_open_server_list, (char *) &pix_book,
        M_MENUPIX, 0, 0, 1, GDK_s
    },
    {0, 0, 0, M_SEP, 0, 0, 0},
...
#ifdef USE_PLUGIN
    {   N_("_Load Plugin or Script..."), menu_loadplugin,
        GTK_STOCK_REVERT_TO_SAVED, M_MENUSTOCK, 0, 0, 1
    },
#else
    {   N_("_Load Plugin or Script..."), 0, GTK_STOCK_REVERT_TO_SAVED,
        M_MENUSTOCK, 0, 0, 0
    },
#endif
...
#define MENUBAR_OFFSET (17)
    ...
    {   N_("_Topic Bar"), menu_topicbar_toggle, 0, M_MENUTOG, MENU_ID_TOPICBAR,
        0, 1
    },
    {   N_("_User List"), menu_userlist_toggle, 0, M_MENUTOG, MENU_ID_USERLIST,
        0, 1, GDK_F7
    },
    {   N_("U_serlist Buttons"), menu_ulbuttons_toggle, 0, M_MENUTOG,
        MENU_ID_ULBUTTONS, 0, 1
    },
    {   N_("M_ode Buttons"), menu_cmbuttons_toggle, 0, M_MENUTOG,
        MENU_ID_MODEBUTTONS, 0, 1
    },
    {0, 0, 0, M_SEP, 0, 0, 0},
...
#define METRE_OFFSET (28)
    ...
    {   N_("_Disconnect"), menu_disconnect, GTK_STOCK_DISCONNECT, M_MENUSTOCK,
        MENU_ID_DISCONNECT, 0, 1
    },
    {   N_("_Reconnect"), menu_reconnect, GTK_STOCK_CONNECT, M_MENUSTOCK,
        MENU_ID_RECONNECT, 0, 1
    },
    {   N_("Join a Channel..."), menu_join, GTK_STOCK_JUMP_TO, M_MENUSTOCK,
        MENU_ID_JOIN, 0, 1
    },
    {   N_("List of Channels..."), menu_chanlist, GTK_STOCK_INDEX, M_MENUITEM, 0,
        0, 1
    },
    {0, 0, 0, M_SEP, 0, 0, 0},
#define AWAY_OFFSET (39)
    ...
    {   N_("_Preferences"), menu_settings, GTK_STOCK_PREFERENCES, M_MENUSTOCK, 0,
        0, 1
    },
    ...
    {   N_("C_lear Text"), menu_flushbuffer, GTK_STOCK_CLEAR, M_MENUSTOCK, 0, 0,
        1, GDK_l
    },
#define SEARCH_OFFSET 67
    {   N_("Search Text..."), menu_search, GTK_STOCK_FIND, M_MENUSTOCK, 0, 0, 1,
        GDK_f
    },
    ...
    {   N_("_Contents"), menu_docs, GTK_STOCK_HELP, M_MENUSTOCK, 0, 0, 1,
        GDK_F1
    },
#if 0
...
static void
menu_foreach_gui (menu_entry * me,
                  void (*callback) (GtkWidget *, menu_entry *, char *))
{
    GSList *list = sess_list;
    int tabdone = FALSE;
    session *sess;
...
static GtkWidget *
menu_radio_item (char *label, GtkWidget * menu, void *callback,
                 void *userdata, int state, char *groupname)
{
    GtkWidget *item;
    GtkMenuItem *parent;
    ...
    if (parent)
        grouplist =
            gtk_radio_menu_item_get_group ((GtkRadioMenuItem *) parent);
...
menu_reorder (GtkMenu * menu, GtkWidget * item, int pos)
{
    ...
    if (pos < 0)                /* position offset from end/bottom */
        gtk_menu_reorder_child (menu, item,
                                (g_list_length
                                 (GTK_MENU_SHELL (menu)->children) + pos) -
                                1);
    else
...
menu_add_radio (GtkWidget * menu, menu_entry * me)
{
    ...
    {
        item =
            menu_radio_item (me->label, menu, menu_radio_cb, me, me->state,
                             me->group);
...
menu_add_toggle (GtkWidget * menu, menu_entry * me)
{
    ...
    {
        item =
            menu_toggle_item (me->label, menu, menu_toggle_cb, me,
                              me->state);
        menu_reorder (GTK_MENU (menu), item, me->pos);
    }
    return item;
...
menu_add_item (GtkWidget * menu, menu_entry * me, char *target)
{
    ...
    {
        item =
            menu_quick_item (me->cmd, me->label, menu,
                             me->
                             markup ? XCMENU_MARKUP | XCMENU_MNEMONIC :
                             XCMENU_MNEMONIC, target, me->icon);
        menu_reorder (GTK_MENU (menu), item, me->pos);
    }
    return item;
}
    ...
    while (1)
    {
        ...
        case M_NEWMENU:
            ...
            if (mymenu[i].id == MENU_ID_USERMENU)
                usermenu = menu;
            menu_item =
                gtk_menu_item_new_with_mnemonic (_(mymenu[i].text));
            /* record the English name for /menu */
            g_object_set_data (G_OBJECT (menu_item), "name",
                               mymenu[i].text);
        ...
        case M_MENUPIX:
            item =
                create_icon_menu (_(mymenu[i].text), mymenu[i].image,
                                  FALSE);
            goto normalitem;
...
normalitem:
        ...
        case M_MENUTOG:
            item =
                gtk_check_menu_item_new_with_mnemonic (_(mymenu[i].text));
togitem:
