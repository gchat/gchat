...
mg_create_tab_colors (void)
{
    ...
    newdata_list =
        mg_attr_list_create (&colors[COL_NEW_DATA], prefs.tab_small);
    nickseen_list =
        mg_attr_list_create (&colors[COL_HILIGHT], prefs.tab_small);
...
set_window_urgency (GtkWidget * win, gboolean set)
{
    ...
    hints =
        XGetWMHints (GDK_WINDOW_XDISPLAY (win->window),
...
static gboolean
mg_inputbox_focus (GtkWidget * widget, GdkEventFocus * event,
                   session_gui * gui)
{
    GSList *list;
    session *sess;
...
fe_set_title (session * sess)
{
    ...
    case SESS_SNOTICES:
        snprintf (tbuf, sizeof (tbuf), DISPLAY_NAME ": %s @ %s (notices)",
                  sess->server->nick, server_get_network (sess->server,
                          TRUE));
        break;
    default:
def:
        gtk_window_set_title (GTK_WINDOW (sess->gui->window),
                              "GANGSTA CHAT");
        return;
    }
...
static gboolean
mg_windowstate_cb (GtkWindow * wid, GdkEventWindowState * event,
                   gpointer userdata)
{
...
mg_configure_cb (GtkWidget * wid, GdkEventConfigure * event, session * sess)
{
    if (sess == NULL)           /* for the main_window */
    {
        if (mg_gui)
        {
            ...
            {
                sess = current_sess;
                gtk_window_get_position (GTK_WINDOW (wid),
                                         &prefs.mainwindow_left,
                                         &prefs.mainwindow_top);
                gtk_window_get_size (GTK_WINDOW (wid),
...
mg_unpopulate (session * sess)
{
    ...
    if (gui->laginfo)
        res->lag_text =
            strdup (gtk_label_get_text (GTK_LABEL (gui->laginfo)));
    if (gui->throttleinfo)
        res->queue_text =
            strdup (gtk_label_get_text (GTK_LABEL (gui->throttleinfo)));
    ...
    if (gui->lagometer)
        res->lag_value =
            gtk_progress_bar_get_fraction (GTK_PROGRESS_BAR (gui->lagometer));
    if (gui->throttlemeter)
        res->queue_value =
            gtk_progress_bar_get_fraction (GTK_PROGRESS_BAR
                                           (gui->throttlemeter));
...
mg_set_topic_tip (session * sess)
{
    ...
    case SESS_CHANNEL:
        ...
        {
            text =
                g_strdup_printf (_("Topic for %s is: %s"), sess->channel,
                                 sess->topic);
    ...
    default:
        if (GTK_ENTRY (sess->gui->topic_entry)->text &&
            ...
            add_tip (sess->gui->topic_entry,
                     GTK_ENTRY (sess->gui->topic_entry)->text);
        else
...
mg_userlist_showhide (session * sess, int show)
{
    ...
    if (show)
    {
        ...
        gtk_widget_style_get (GTK_WIDGET (gui->hpane_right), "handle-size",
                              &handle_size, NULL);
        gtk_paned_set_position (GTK_PANED (gui->hpane_right),
                                GTK_WIDGET (gui->hpane_right)->allocation.
                                width - (prefs.gui_pane_right_size +
                                         handle_size));
    }
    else
    {
...
mg_is_userlist_and_tree_combined (void)
{
    ...
    if (prefs.tab_pos == POS_TOPRIGHT
            && prefs.gui_ulist_pos == POS_BOTTOMRIGHT)
        return TRUE;
    if (prefs.tab_pos == POS_BOTTOMRIGHT
            && prefs.gui_ulist_pos == POS_TOPRIGHT)
        return TRUE;
...
mg_populate_userlist (session * sess)
{
    ...
    if (is_session (sess))
    {
        ...
        if (sess->type == SESS_DIALOG)
            ...
            mg_set_access_icon (sess->gui,
                                get_user_icon (sess->server, sess->me),
                                sess->server->is_away);
        userlist_show (sess);
...
mg_populate (session * sess)
{
    ...
    if (strcmp
            (sess->server->nick,
             gtk_button_get_label (GTK_BUTTON (gui->nick_label))) != 0)
    ...
    /* menu items */
    GTK_CHECK_MENU_ITEM (gui->menu_item[MENU_ID_AWAY])->active =
        sess->server->is_away;
    gtk_widget_set_sensitive (gui->menu_item[MENU_ID_AWAY],
                              sess->server->connected);
    gtk_widget_set_sensitive (gui->menu_item[MENU_ID_JOIN],
                              sess->server->end_of_motd);
    gtk_widget_set_sensitive (gui->menu_item[MENU_ID_DISCONNECT],
                              sess->server->connected
                              || sess->server->recondelay_tag);
...
mg_tab_close (session * sess)
{
    ...
    else
    {
        ...
        dialog = gtk_message_dialog_new (GTK_WINDOW (parent_window), 0,
                                         ...
                                         _
                                         ("This server still has %d channels or dialogs associated with it. "
                                          "Close them all?"), i);
...
mg_open_quit_dialog (gboolean minimize_button)
{
    ...
    gtk_window_set_title (GTK_WINDOW (dialog), _("Quit GChat?"));
    ...
    text =
        ...
        ("<span weight=\"bold\" size=\"larger\">%s</span>\n\n%s\n%s",
         _("Are you sure you want to quit?"), cons ? connecttext : "",
...
mg_destroy_tab_cb (GtkWidget * item, chan * ch)
{
    ...
    mg_xbutton_cb (mg_gui->chanview, ch, chan_get_tag (ch),
...
mg_create_color_menu (GtkWidget * menu, session * sess)
{
    ...
    for (i = 0; i < 8; i++)
    {
        sprintf (buf,
                 ...
                 "   </span></tt>", i, colors[i].red >> 8,
                 colors[i].green >> 8, colors[i].blue >> 8);
...
static void
mg_perchan_menu_item (char *label, GtkWidget * menu, guint8 * setting,
                      guint global)
{
...
mg_create_perchannelmenu (session * sess, GtkWidget * menu)
{
    ...
    submenu =
        menu_quick_sub (_("_Settings"), menu, NULL, XCMENU_MNEMONIC, -1);

    mg_perchan_menu_item (_("_Log to Disk"), submenu, &sess->text_logging,
                          prefs.logging);
    mg_perchan_menu_item (_("_Reload Scrollback"), submenu,
                          &sess->text_scrollback, prefs.text_replay);
    if (sess->type == SESS_CHANNEL)
        mg_perchan_menu_item (_("_Hide Join/Part Messages"), submenu,
...
mg_create_alertmenu (session * sess, GtkWidget * menu)
{
    ...
    submenu =
        menu_quick_sub (_("_Extra Alerts"), menu, NULL, XCMENU_MNEMONIC, -1);

    mg_perchan_menu_item (_("Beep on _Message"), submenu, &sess->alert_beep,
                          prefs.input_beep_chans);
    mg_perchan_menu_item (_("Blink Tray _Icon"), submenu, &sess->alert_tray,
                          prefs.input_tray_chans);
    mg_perchan_menu_item (_("Blink Task _Bar"), submenu, &sess->alert_taskbar,
                          prefs.input_flash_chans);
}
...
mg_create_tabmenu (session * sess, GdkEventButton * event, chan * ch)
{
    ...
    {
        char *name =
            g_markup_escape_text (sess->channel[0] ? sess->
                                  channel : _("<none>"), -1);
    ...
    while (*p)
    {
        ...
        if (strncasecmp ("file:", p, 5) == 0)
        {
            ...
            if (fname)
            {
                ...
                if (p)
                {
                    dcc_send (sess, target, p, prefs.dcc_max_send_cps,
                              0);
                    g_free (p);
                }
                g_free (fname);
                    ...
                    guint32 time, gpointer ud)
{
    if (current_sess->type == SESS_DIALOG)
        ...
        mg_dnd_drop_file (current_sess, current_sess->channel,
                          selection_data->data);
}
...
mg_add_chan (session * sess)
{
    ...
    sess->res->tab =
        chanview_add (sess->gui->chanview, name, sess->server, sess,
                      sess->type == SESS_SERVER ? FALSE : TRUE, TAG_IRC,
                      icon);
    if (plain_list == NULL)
    ...
    {
        sess->res->buffer =
            gtk_xtext_buffer_new (GTK_XTEXT (sess->gui->xtext));
...
mg_create_userlistbuttons (GtkWidget * box)
{
    ...
    while (list)
    {
        ...
        if (pop->cmd[0])
        {
            mg_userlist_button (tab, pop->name, pop->cmd, a, a + 1, b,
                                b + 1);
            a++;
            if (a == 2)
            {
...
mg_link_irctab (session * sess, int focus)
{
    ...
    ((xtext_buffer *) sess->res->buffer)->xtext =
        (GtkXText *) sess->gui->xtext;
    if (win)
        gtk_widget_destroy (win);
}
...
mg_flagbutton_cb (GtkWidget * but, char *flag)
{
    ...
    case 'b':
        ignore_chanmode = TRUE;
        gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (sess->gui->flag_b),
                                      FALSE);
        ignore_chanmode = FALSE;
...
mg_limit_entry_cb (GtkWidget * igad, gpointer userdata)
{
    ...
    if (serv->connected && sess->channel[0])
    {
        if (check_is_number ((char *) gtk_entry_get_text (GTK_ENTRY (igad)))
                == FALSE)
        {
...
mg_word_check (GtkWidget * xtext, char *word, int len)
{
    ...
    if (ret == 0)
    {
        if (((word[0] == '@' || word[0] == '+' || word[0] == '%')
                && userlist_find (sess, word + 1))
                || userlist_find (sess, word))
            return WORD_NICK;
...
mg_word_clicked (GtkWidget * xtext, char *word, GdkEventButton * even)
{
    ...
    case WORD_HOST:
        ...
        break;
    case WORD_NICK:
        menu_nickmenu (sess, even,
                       (word[0] == '@' || word[0] == '+'|| word[0] == '%') ? word + 1 : word, FALSE);
        break;
    case WORD_CHANNEL:
...
mg_update_xtext (GtkWidget * wid)
{
    ...
    gtk_xtext_set_tint (xtext, prefs.tint_red, prefs.tint_green,
                        prefs.tint_blue);
    ...
    gtk_xtext_set_show_separator (xtext,
                                  prefs.indent_nicks ? prefs.
                                  show_separator : 0);
    gtk_xtext_set_indent (xtext, prefs.indent_nicks);
    if (!gtk_xtext_set_font (xtext, prefs.font_normal))
    {
        fe_message ("Failed to open any font. I'm out of here!",
                    FE_MSG_WAIT | FE_MSG_ERROR);
        exit (1);
    }
...
mg_rightpane_cb (GtkPaned * pane, GParamSpec * param, session_gui * gui)
{
    ...
    gtk_widget_style_get (GTK_WIDGET (pane), "handle-size", &handle_size,
                          NULL);
    /* record the position from the RIGHT side */
    prefs.gui_pane_right_size =
        GTK_WIDGET (pane)->allocation.width - gtk_paned_get_position (pane) -
        handle_size;
}
...
mg_create_center (session * sess, session_gui * gui, GtkWidget * box)
{
    ...
    gtk_paned_set_position (GTK_PANED (gui->hpane_left),
                            prefs.gui_pane_left_size);
    ...
    if (prefs.tweak_swap)
    {
        gtk_paned_pack2 (GTK_PANED (gui->hpane_left), gui->vpane_left,
                         FALSE, TRUE);
        gtk_paned_pack1 (GTK_PANED (gui->hpane_left), gui->hpane_right,
                         TRUE, TRUE);
    }
    else
    {
        gtk_paned_pack1 (GTK_PANED (gui->hpane_left), gui->vpane_left,
                         FALSE, TRUE);
        gtk_paned_pack2 (GTK_PANED (gui->hpane_left), gui->hpane_right,
                         TRUE, TRUE);
    }
    gtk_paned_pack2 (GTK_PANED (gui->hpane_right), gui->vpane_right, FALSE,
                     TRUE);
...
static void
mg_place_userlist_and_chanview_real (session_gui * gui, GtkWidget * userlist,
                                     GtkWidget * chanview)
{
    ...
    if (chanview)
    {
        ...
        {
        case POS_TOPLEFT:
            gtk_paned_pack1 (GTK_PANED (gui->vpane_left), chanview, FALSE,
                             TRUE);
            break;
        case POS_BOTTOMLEFT:
            gtk_paned_pack2 (GTK_PANED (gui->vpane_left), chanview, FALSE,
                             TRUE);
            break;
        case POS_TOPRIGHT:
            gtk_paned_pack1 (GTK_PANED (gui->vpane_right), chanview,
                             FALSE, TRUE);
            break;
        case POS_BOTTOMRIGHT:
            gtk_paned_pack2 (GTK_PANED (gui->vpane_right), chanview,
                             FALSE, TRUE);
            break;
        case POS_TOP:
            gtk_table_set_row_spacing (GTK_TABLE (gui->main_table), 1,
                                       GUI_SPACING - 1);
            gtk_table_attach (GTK_TABLE (gui->main_table), chanview, 1, 2,
    ...
    if (userlist)
    {
        ...
        {
        case POS_TOPLEFT:
            gtk_paned_pack1 (GTK_PANED (gui->vpane_left), userlist, FALSE,
                             TRUE);
            break;
        case POS_BOTTOMLEFT:
            gtk_paned_pack2 (GTK_PANED (gui->vpane_left), userlist, FALSE,
                             TRUE);
            break;
        case POS_BOTTOMRIGHT:
            gtk_paned_pack2 (GTK_PANED (gui->vpane_right), userlist,
                             FALSE, TRUE);
            break;
            /*case POS_HIDDEN:
               break; *//* Hide using the VIEW menu instead */
        default:           /* POS_TOPRIGHT */
            gtk_paned_pack1 (GTK_PANED (gui->vpane_right), userlist,
                             FALSE, TRUE);
        }
    }
...
mg_place_userlist_and_chanview (session_gui * gui)
{
    ...
    if (gui->chanview)
    {
        ...
        else if ((pos == POS_TOPLEFT || pos == POS_BOTTOMLEFT
                  || pos == POS_TOPRIGHT || pos == POS_BOTTOMRIGHT)
...
mg_change_layout (int type)
{
    if (mg_gui)
    {
        ...
        if (type == 0 && prefs.tab_pos != POS_BOTTOM
                && prefs.tab_pos != POS_TOP)
...
mg_create_tabs (session_gui * gui)
{
    ...
    gui->chanview = chanview_new (prefs.tab_layout, prefs.truncchans,
                                  ...
                                  prefs.
                                  style_namelistgad ? input_style : NULL);
...
mg_tabwin_focus_cb (GtkWindow * win, GdkEventFocus * event, gpointer userdata)
{
    ...
    if (current_sess)
    {
        gtk_xtext_check_marker_visibility (GTK_XTEXT
                                           (current_sess->gui->xtext));
...
mg_create_topwindow (session * sess)
{
    ...
    if (sess->res->buffer == NULL)
    {
        sess->res->buffer =
            gtk_xtext_buffer_new (GTK_XTEXT (sess->gui->xtext));
        gtk_xtext_buffer_show (GTK_XTEXT (sess->gui->xtext),
                               sess->res->buffer, TRUE);
        gtk_xtext_set_time_stamp (sess->res->buffer, prefs.timestamp);
        sess->res->user_model = userlist_create_model ();
    }
    ...
    if (sess->gui->is_tab)
    {
        if (sess->waitchannel[0])
        {
            if (prefs.truncchans > 2
                    && g_utf8_strlen (sess->waitchannel,
                                      -1) > prefs.truncchans)
            {
                /* truncate long channel names */
                tbuf[0] = '(';
                strcpy (tbuf + 1, sess->waitchannel);
                g_utf8_offset_to_pointer (tbuf, prefs.truncchans)[0] =
                    0;
                strcat (tbuf, "..)");
            }
            else
...
fe_update_mode_buttons (session * sess, char mode, char sign)
{
    ...
    for (i = 0; i < NUM_FLAG_WIDS - 1; i++)
    {
        if (chan_flags[i] == mode)
        {
            ...
            {
                ignore_chanmode = TRUE;
                if (GTK_TOGGLE_BUTTON (sess->gui->flag_wid[i])->
                        active != state)
                    gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON
                                                  (sess->gui->
                                                   flag_wid[i]), state);
...
fe_set_away (server * serv)
{
    ...
    while (list)
    {
        ...
        if (sess->server == serv)
        {
            if (!sess->gui->is_tab || sess == current_tab)
            {
                GTK_CHECK_MENU_ITEM (sess->gui->
                                     menu_item[MENU_ID_AWAY])->active =
                                         serv->is_away;
...
gboolean
mg_drag_begin_cb (GtkWidget * widget, GdkDragContext * context,
                  gpointer userdata)
{
    ...
    pix =
        gdk_pixbuf_get_from_drawable (NULL, widget->window, cmap, 0, 0, 0, 0,
                                      width, height);
    pix2 =
        gdk_pixbuf_scale_simple (pix, width * 4 / 5, height / 2,
                                 GDK_INTERP_HYPER);
    g_object_unref (pix);
...
void
mg_drag_end_cb (GtkWidget * widget, GdkDragContext * context,
                gpointer userdata)
{
    /* ignore file drops */
    if (!mg_is_gui_target (context))
...
gboolean
mg_drag_drop_cb (GtkWidget * widget, GdkDragContext * context, int x, int y,
...
gboolean
mg_drag_motion_cb (GtkWidget * widget, GdkDragContext * context, int x, int y,
                   guint time, gpointer scbar)
{
    GdkGC *gc;
    GdkColor col;
    GdkGCValues val;
    ...
    gc = gdk_gc_new_with_values (widget->window, &val,
    ...
    if (y < half)
    {
        gdk_draw_rectangle (draw, gc, FALSE, 1 + ox, 2 + oy, width - 3,
                            half - 4);
        gdk_draw_rectangle (draw, gc, FALSE, 0 + ox, 1 + oy, width - 1,
                            half - 2);
    ...
    {
        gdk_draw_rectangle (draw, gc, FALSE, 0 + ox, half + 1 + oy,
                            width - 1, half - 2);
        gdk_draw_rectangle (draw, gc, FALSE, 1 + ox, half + 2 + oy,
                            width - 3, half - 4);
