...
static void servlist_network_row_cb (GtkTreeSelection * sel,
                                     gpointer user_data);
    ...
    while (list)
      {
          ...
          if (net->selected == i)
              /* select this server */
              servlist_select_and_show (GTK_TREE_VIEW (treeview), &iter,
                                        store);
...
servlist_addserver_cb (GtkWidget * item, GtkWidget * treeview)
{
    ...
    servlist_server_row_cb (gtk_tree_view_get_selection
                            (GTK_TREE_VIEW (networks_tree)), NULL);
}
...
servlist_addnet_cb (GtkWidget * item, GtkTreeView * treeview)
{
    ...
    servlist_network_row_cb (gtk_tree_view_get_selection
                             (GTK_TREE_VIEW (networks_tree)), NULL);
}
    ...
    if (pos >= 0)
      {
          ...
          if (pos >= 0)
            {
                selected_net->servlist =
                    g_slist_remove (selected_net->servlist, serv);
                selected_net->servlist =
                    g_slist_insert (selected_net->servlist, serv, pos);
                ...
                if (net_name)
                  {
                      if (toupper (net_name[0]) == c)
                        {
                            servlist_select_and_show (tree, &iter,
                                                      GTK_LIST_STORE (model));
                            g_free (net_name);
                            return TRUE;
                        }
...
static gboolean
servlist_serv_keypress_cb (GtkWidget * wid, GdkEventKey * evt,
                           gpointer userdata)
{
...
{
    network_list =
        g_slist_sort (network_list, (GCompareFunc) servlist_compare);
    servlist_networks_populate (networks_tree, network_list);
}
...
static gint
servlist_editwin_delete_cb (GtkWidget * win, GdkEventAny * event,
                            gpointer none)
{
...
static gboolean
servlist_configure_cb (GtkWindow * win, GdkEventConfigure * event,
                       gpointer none)
{
...
servlist_edit_cb (GtkWidget * but, gpointer none)
{
    ...
    g_signal_connect (G_OBJECT
                      (gtk_tree_view_get_selection
                       (GTK_TREE_VIEW (edit_tree))), "changed",
                      G_CALLBACK (servlist_server_row_cb), NULL);
...
static gboolean
servlist_get_iter_from_name (GtkTreeModel * model, gchar * name,
                             GtkTreeIter * iter)
{
...
static void
servlist_editchannel_cb (GtkCellRendererText * cell, gchar * name,
                         gchar * newval, GtkTreeModel * model)
{
    GtkTreeIter iter;
...
static void
servlist_editkey_cb (GtkCellRendererText * cell, gchar * name, gchar * newval,
                     GtkTreeModel * model)
{
    GtkTreeIter iter;
...
servlist_autojoinedit (ircnet * net, char *channel, gboolean add)
{
    ...
    gtk_window_set_title (GTK_WINDOW (win),
                          _("GChat: Favorite Channels (Auto-Join List)"));
    ...
    snprintf (buf, sizeof (buf),
              _("These channels will be joined whenever you connect to %s."),
              net->name);
    ...
    gtk_table_attach (GTK_TABLE (table), label2, 0, 2, 1, 2, GTK_FILL, 0, 3,
                      3);
    gtk_widget_show (label2);

    scrolledwindow = gtk_scrolled_window_new (NULL, NULL);
    gtk_table_attach (GTK_TABLE (table), scrolledwindow, 0, 1, 2, 3,
    ...
    gtk_tree_view_column_set_expand (gtk_tree_view_get_column
                                     (GTK_TREE_VIEW (tree), 0), TRUE);
    gtk_tree_view_column_set_expand (gtk_tree_view_get_column
                                     (GTK_TREE_VIEW (tree), 1), TRUE);

    gtk_tree_sortable_set_sort_column_id ((GtkTreeSortable *) model, 0,
                                          GTK_SORT_ASCENDING);
    ...
    gtk_table_attach (GTK_TABLE (table), vbuttonbox1, 1, 2, 2, 3, GTK_FILL,
                      GTK_FILL, 3, 0);
    ...
    gtk_table_attach (GTK_TABLE (table), bbox, 0, 1, 3, 4,
                      GTK_FILL | GTK_EXPAND, GTK_FILL, 0, 4);
    gtk_widget_show (bbox);

    wid = gtk_button_new_from_stock (GTK_STOCK_CANCEL);
    g_signal_connect (G_OBJECT (wid), "clicked", G_CALLBACK (gtkutil_destroy),
                      win);
    ...
    g_signal_connect (G_OBJECT (wid), "clicked",
                      G_CALLBACK (servlist_autojoineditok_cb), tree);
    gtk_container_add (GTK_CONTAINER (bbox), wid);
    gtk_widget_show (wid);
          ...
          while (clist)
            {
                if (channel && !add && !rfc_casecmp (channel, clist->data))
                  {
                      snprintf (buf, sizeof (buf), _("%s has been removed."),
                                channel);
                      snprintf (lab, sizeof (lab),
                                "<span foreground=\"#2222DD\">%s</span>",
                                buf);
                  ...
                  {
                      gtk_list_store_append (store, &iter);
                      gtk_list_store_set (store, &iter, 0, clist->data, 1,
                                          klist->data, 2, TRUE, -1);
                  }
...
servlist_connect_cb (GtkWidget * button, gpointer userdata)
{
    ...
    if (servlist_savegui () != 0)
      {
          fe_message (_("User name and Real name cannot be left blank."),
                      FE_MSG_ERROR);
          return;
      }
    ...
    if (net)
      {
          ...
          gtk_list_store_set (GTK_LIST_STORE (model), &iter, 0, net->name,
                              -1);
          free (netname);
      }
...
static GtkWidget *
servlist_create_check (int num, int state, GtkWidget * table, int row,
                       int col, char *labeltext)
{
    GtkWidget *but;
    ...
    if (row == 15)              /* for "Channels to Join:" */
      {
          ...
          g_signal_connect (G_OBJECT (button), "clicked",
                            G_CALLBACK (servlist_autojoinedit_cb),
                            selected_net);
...
static void
servlist_editserver_cb (GtkCellRendererText * cell, gchar * arg1,
...
servlist_open_edit (GtkWidget * parent, ircnet * net)
{
    ...
    snprintf (buf, sizeof (buf), _("GChat: Edit %s"), net->name);
    ...
    gtk_window_set_type_hint (GTK_WINDOW (editwindow),
    ...
    add_tip (check,
             _
             ("Don't cycle through all the servers when the connection fails."));
    ...
    servlist_create_check (3, net->flags & FLAG_AUTO_CONNECT, table3,
                           11, 1,
                           _("Auto connect to this network at startup"));
    servlist_create_check (4, !(net->flags & FLAG_USE_PROXY), table3, 12, 1,
                           _("Bypass proxy server"));
    check =
        servlist_create_check (2, net->flags & FLAG_USE_SSL, table3, 13, 1,
                               _
    ...
    check = servlist_create_check (5, net->flags & FLAG_ALLOW_INVALID, table3,
                                   14, 1,
                                   _("Accept invalid SSL certificate"));
#ifndef USE_OPENSSL
    gtk_widget_set_sensitive (check, FALSE);
#endif
    ...
    edit_entry_cmd =
        servlist_create_entry (table3, _("Connect command:"), 16,
                               net->command, 0,
                               _
                               ("Extra command to execute after connecting. If you need more than one, set this to LOAD -e <filename>, where <filename> is a text-file full of commands to execute."));

    edit_entry_nickserv =
        servlist_create_entry (table3, _("Nickserv password:"), 17,
                               net->nickserv, 0,
                               _
                               ("If your nickname requires a password, enter it here. Not all IRC networks support this."));
    ...
    edit_entry_pass =
        servlist_create_entry (table3, _("Server password:"), 18,
                               net->pass, 0,
                               _
                               ("Password for the server, if in doubt, leave blank."));
    ...
    ignore_changed = TRUE;
    gtk_entry_set_text (GTK_ENTRY (GTK_BIN (comboboxentry_charset)->child),
                        net->encoding ? net->encoding : "System default");
...
servlist_open_networks (void)
{
    ...
    gtk_window_set_title (GTK_WINDOW (servlist), _("Gangsta Networks"));
    gtk_window_set_default_size (GTK_WINDOW (servlist), win_width,
                                 win_height);
    gtk_window_set_position (GTK_WINDOW (servlist), GTK_WIN_POS_MOUSE);
    gtk_window_set_role (GTK_WINDOW (servlist), "servlist");
    gtk_window_set_type_hint (GTK_WINDOW (servlist),
                              GDK_WINDOW_TYPE_HINT_DIALOG);
    if (current_sess)
        gtk_window_set_transient_for (GTK_WINDOW (servlist),
                                      GTK_WINDOW (current_sess->gui->window));
    ...
    gtk_button_box_set_layout (GTK_BUTTON_BOX (vbuttonbox2),
                               GTK_BUTTONBOX_START);
...
fe_serverlist_open (session * sess)
{
    ...
    g_signal_connect (G_OBJECT
                      (gtk_tree_view_get_selection
                       (GTK_TREE_VIEW (networks_tree))), "changed",
                      G_CALLBACK (servlist_network_row_cb), NULL);
