...
{
    model_foreach_1 (GTK_TREE_MODEL (cv->store), (void *) chanview_pop_cb,
                     cv);
}
...
{
    model_foreach_1 (GTK_TREE_MODEL (cv->store), (void *) chanview_free_ch,
                     cv);
...
static void
chanview_insert_sorted (chanview * cv, GtkTreeIter * add_iter,
                        GtkTreeIter * parent, void *ud)
{
    GtkTreeIter iter;
    chan *ch;
              ...
              gboolean allow_closure, int tag, GdkPixbuf * icon)
{
    ...
    ret =
        chanview_add_real (cv, new_name, family, userdata, allow_closure, tag,
                           icon, NULL, NULL);
