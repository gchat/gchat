 ...
 */
static void
notify_treecell_property_mapper (GtkTreeViewColumn * col,
                                 ...
                                 GtkTreeIter * iter, gpointer data)
{
    ...
    gtk_tree_model_get (GTK_TREE_MODEL (model), iter,
                        COLOUR_COLUMN, &colour, model_column, &text, -1);
...
notify_treeview_new (GtkWidget * box)
{
    ...
    store = gtk_list_store_new (N_COLUMNS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_POINTER,  /* can't specify colour! */
                                G_TYPE_POINTER);
...
fe_notify_ask (char *nick, char *networks)
{
    ...
    snprintf (buf, sizeof (buf), "<i><span size=\"smaller\">%s</span></i>",
              _("Comma separated list of networks is accepted."));
...
notify_opengui (void)
{
    ...
    notify_window =
        mg_create_generic_tab ("Notify", _("XChat: Friends List"), FALSE,
                               TRUE, notify_closegui, NULL, 400, 250, &vbox,
                               0);
