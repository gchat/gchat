...
search_open (session * sess)
{
    ...
    last = NULL;
    win = mg_create_generic_tab ("search", _("GChat: Search"), TRUE, FALSE,
    ...
    g_signal_connect (G_OBJECT (wid), "toggled",
                      G_CALLBACK (search_caseign_cb), sess);
    ...
    g_signal_connect (G_OBJECT (wid), "toggled",
                      G_CALLBACK (search_dirbwd_cb), sess);
    gtk_container_add (GTK_CONTAINER (vbox), wid);
    gtk_widget_show (wid);
