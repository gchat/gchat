...
static void (*enchant_broker_free_dict) (struct EnchantBroker * broker,
        struct EnchantDict * dict);
static void (*enchant_broker_list_dicts) (struct EnchantBroker * broker,
        EnchantDictDescribeFn fn,
        void *user_data);
static struct EnchantDict *(*enchant_broker_request_dict) (struct
        EnchantBroker *
        broker,
        const char *const
        tag);

static void (*enchant_dict_add_to_personal) (struct EnchantDict * dict,
        const char *const word,
        ssize_t len);
static void (*enchant_dict_add_to_session) (struct EnchantDict * dict,
        const char *const word,
        ssize_t len);
static int (*enchant_dict_check) (struct EnchantDict * dict,
                                  const char *const word, ssize_t len);
static void (*enchant_dict_describe) (struct EnchantDict * dict,
                                      EnchantDictDescribeFn fn,
                                      void *user_data);
static void (*enchant_dict_free_suggestions) (struct EnchantDict * dict,
        char **suggestions);
static void (*enchant_dict_store_replacement) (struct EnchantDict * dict,
        const char *const mis,
        ssize_t mis_len,
        const char *const cor,
        ssize_t cor_len);
static char **(*enchant_dict_suggest) (struct EnchantDict * dict,
                                       const char *const word, ssize_t len,
                                       size_t * out_n_suggs);
...
static gint sexy_spell_entry_expose (GtkWidget * widget,
                                     GdkEventExpose * event);
static gint sexy_spell_entry_button_press (GtkWidget * widget,
...
/* Other handlers */
static gboolean sexy_spell_entry_popup_menu (GtkWidget * widget,
...
static gboolean default_word_check (SexySpellEntry * entry,
                                    const gchar * word);
static gboolean sexy_spell_entry_activate_language_internal (SexySpellEntry *
        entry,
        const gchar *
        lang,
        GError ** error);
...
static void entry_strsplit_utf8 (GtkEntry * entry,
                                 gchar *** set, gint ** starts, gint ** ends);
...
#define MODULE_SYMBOL(name, func) \
    ...
    MODULE_SYMBOL ("enchant_broker_request_dict",
                   enchant_broker_request_dict)
    MODULE_SYMBOL ("enchant_dict_add_to_personal",
                   enchant_dict_add_to_personal)
    MODULE_SYMBOL ("enchant_dict_add_to_session",
                   enchant_dict_add_to_session)
    MODULE_SYMBOL ("enchant_dict_check",
                   enchant_dict_check)
    MODULE_SYMBOL ("enchant_dict_describe",
...
static void
get_word_extents_from_position (SexySpellEntry * entry, gint * start,
...
static void
build_suggestion_menu (SexySpellEntry * entry, GtkWidget * menu,
                       struct EnchantDict *dict, const gchar * word)
{
    GtkWidget *mi;
    gchar **suggestions;
...
build_spelling_menu (SexySpellEntry * entry, const gchar * word)
{
    ...
    else
    {
        ...
        for (li = entry->priv->dict_list; li; li = g_slist_next (li))
        {
            ...
            g_signal_connect (G_OBJECT (submi), "activate",
                              G_CALLBACK (add_to_dictionary), entry);
...
static void
sexy_spell_entry_populate_popup (SexySpellEntry * entry, GtkMenu * menu,
                                 gpointer data)
{
    GtkWidget *icon, *mi;
    gint start, end;
    ...
    gtk_menu_item_set_submenu (GTK_MENU_ITEM (mi),
                               build_spelling_menu (entry, word));
    g_free (word);
...
check_word (SexySpellEntry * entry, int start, int end)
{
    ...
    do
    {
        ...
        if (s == start)
        {
            ...
            g_slist_foreach (attrs, (GFunc) pango_attribute_destroy,
                             NULL);
            g_slist_free (attrs);
        }
    }
...
sexy_spell_entry_recheck_all (SexySpellEntry * entry)
{
    ...
    for (i = 0; entry->priv->words[i]; i++)
    {
        ...
        if (length == 0)
            continue;
        check_word (entry, entry->priv->word_starts[i],
...
static void
entry_strsplit_utf8 (GtkEntry * entry, gchar *** set, gint ** starts,
                     gint ** ends)
{
    PangoLayout *layout;
    ...
    *ends = g_new0 (gint, n_strings);
    ...
    for (i = 0, j = 0; i < n_attrs; i++)
    {
        if (log_attrs[i].is_word_start)
        {
            ...
            bytes =
                (gint) (g_utf8_offset_to_pointer (text, cend) - start);
...
sexy_spell_entry_activate_default_languages (SexySpellEntry * entry)
{
    ...
    for (i = 0; langs[i]; i++)
    {
        ...
        {
            if ((lastprefix == NULL)
                    || (g_str_has_prefix (langs[i], lastprefix) == FALSE))
...
static gboolean
sexy_spell_entry_activate_language_internal (SexySpellEntry * entry,
        const gchar * lang,
        GError ** error)
{
    struct EnchantDict *dict;
    ...
    if (!dict)
    {
        g_set_error (error, SEXY_SPELL_ERROR, SEXY_SPELL_ERROR_BACKEND,
                     _("enchant error for language: %s"), lang);
        return FALSE;
    }

    entry->priv->dict_list =
        g_slist_append (entry->priv->dict_list, (gpointer) dict);
    g_hash_table_insert (entry->priv->dict_hash, get_lang_from_dict (dict),
                         (gpointer) dict);
                                      ...
                                      const gchar * lang)
{
    ...
    if (lang)
    {
        ...
        enchant_broker_free_dict (entry->priv->broker, dict);
        entry->priv->dict_list =
            g_slist_remove (entry->priv->dict_list, dict);
 ...
 */
gboolean
sexy_spell_entry_set_active_languages (SexySpellEntry * entry, GSList * langs,
                                       GError ** error)
{
    GSList *li;
