    ...
    if (pre && pre != serv->nick_prefixes)
    {
        ...
        while (1)
        {
            if (pre[0] == user->prefix[0])
            {
                ...
                {
                case 0:
                    return pix_red;     /* 1 level above op */
                case 1:
                    return pix_purple;  /* 2 levels above op */
                }
                break;    /* 3+, no icons */
            }
            level++;
...
fe_userlist_numbers (session * sess)
{
    ...
    if (sess == current_tab || !sess->gui->is_tab)
    {
        if (sess->total)
        {
            snprintf (tbuf, sizeof (tbuf), _("%d ops, %d total"),
                      sess->ops, sess->total);
...
static void
scroll_to_iter (GtkTreeIter * iter, GtkTreeView * treeview,
                GtkTreeModel * model)
{
...
fe_userlist_set_selected (struct session *sess)
{
    ...
    if (store !=
            (GtkListStore *)
            gtk_tree_view_get_model (GTK_TREE_VIEW (sess->gui->user_tree)))
        return;
    ...
    if (gtk_tree_model_get_iter_first (GTK_TREE_MODEL (store), &iter))
    {
        do
        {
            gtk_tree_model_get (GTK_TREE_MODEL (store), &iter, COL_USER,
                                &user, -1);
...
{
    gtk_adjustment_set_value (gtk_tree_view_get_vadjustment
                              (GTK_TREE_VIEW (treeview)), val);
}
                   ...
                   guint info, guint ttime, gpointer userdata)
{
    ...
    if (!gtk_tree_view_get_path_at_pos
            (widget, x, y, &path, NULL, NULL, NULL))
        return;
...
{
    return gtk_list_store_new (5, GDK_TYPE_PIXBUF, G_TYPE_STRING,
...
userlist_add_columns (GtkTreeView * treeview)
{
    ...
    gtk_tree_view_insert_column_with_attributes (GTK_TREE_VIEW (treeview), -1,
            NULL, renderer, "text", 1,
            "foreground-gdk", 4, NULL);
...
static gint
userlist_click_cb (GtkWidget * widget, GdkEventButton * event,
                   gpointer userdata)
{
    char **nicks;
    int i;
            ...
            event->type == GDK_2BUTTON_PRESS && prefs.doubleclickuser[0])
    {
        ...
        if (nicks)
        {
            nick_command_parse (current_sess, prefs.doubleclickuser,
                                nicks[0], nicks[0]);
            while (i)
            {
                i--;
...
userlist_create (GtkWidget * box)
{
    ...
    gtk_drag_source_set (treeview, GDK_BUTTON1_MASK, dnd_src_target, 1,
                         GDK_ACTION_MOVE);
...
fe_uselect (session * sess, char *word[], int do_clear, int scroll_to)
{
    ...
    if (gtk_tree_model_get_iter_first (model, &iter))
    {
        ...
        do
        {
            if (*word[0])
            {
                gtk_tree_model_get (model, &iter, COL_USER, &row_user,
                                    -1);
                thisname = 0;
                while (*(name = word[thisname++]))
                {
                    if (sess->server->p_cmp (row_user->nick, name) ==
                            0)
                    {
