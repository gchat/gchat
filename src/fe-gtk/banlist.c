...
static void
banlist_add_selected_cb (GtkTreeModel * model, GtkTreePath * path,
...
banlist_opengui (struct session *sess)
{
    ...
    gtkutil_button (bbox, GTK_STOCK_REMOVE, 0, banlist_unban, sess,
                    _("Remove"));
    gtkutil_button (bbox, GTK_STOCK_REMOVE, 0, banlist_crop, sess, _("Crop"));
    gtkutil_button (bbox, GTK_STOCK_CLEAR, 0, banlist_clear, sess,
                    _("Clear"));

    sess->res->banlist_butRefresh =
        gtkutil_button (bbox, GTK_STOCK_REFRESH, 0, banlist_refresh, sess,
                        _("Refresh"));
