 ...
 */
static void
chanlist_place_row_in_gui (server * serv, chanlistrow * next_row,
                           gboolean force)
{
    GtkTreeModel *model;
    ...
    if (force || serv->gui->chanlist_channels_shown_count < 20)
      {
        ...
        serv->gui->chanlist_pending_rows =
            g_slist_prepend (serv->gui->chanlist_pending_rows, next_row);
...
chanlist_get_selected (server * serv, gboolean get_topic)
{
    ...
    gtk_tree_model_get (model, &iter, get_topic ? COL_TOPIC : COL_CHANNEL,
                        &chan, -1);
    return chan;
}
...
chanlist_save (GtkWidget * wid, server * serv)
{
    ...
    if (gtk_tree_model_get_iter_first (model, &iter))
        gtkutil_file_req (_("Select an output filename"),
                          chanlist_filereq_done, serv, NULL, FRF_WRITE);
}
...
chanlist_flash (server * serv)
{
    if (serv->gui->chanlist_refresh->state != GTK_STATE_ACTIVE)
        ...
        gtk_widget_set_state (serv->gui->chanlist_refresh,
                              GTK_STATE_PRELIGHT);
...
static void
chanlist_add_column (GtkWidget * tree, int textcol, int size, char *title,
...
chanlist_opengui (server * serv, int do_refresh)
{
    ...
    serv->gui->chanlist_window =
        mg_create_generic_tab ("ChanList", tbuf, FALSE, TRUE,
                               chanlist_closegui, serv, 640, 480, &vbox,
                               serv);
    ...
    wid =
        gtkutil_button (NULL, GTK_STOCK_FIND, 0, chanlist_search_pressed,
                        serv, _("_Search"));
    ...
    serv->gui->chanlist_tag =
        g_timeout_add (250, (GSourceFunc) chanlist_timeout, serv);
