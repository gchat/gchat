...
remote_object_init (RemoteObject * obj)
{
    obj->hooks =
        g_hash_table_new_full (g_int_hash,
                               g_int_equal, NULL, hook_info_destroy);

    obj->lists =
        g_hash_table_new_full (g_int_hash,
                               g_int_equal, g_free, list_info_destroy);
    obj->dbus_path = NULL;
    obj->filename = NULL;
    obj->last_hook_id = 0;
    obj->last_list_id = 0;
...
remote_object_class_init (RemoteObjectClass * klass)
{
    ...
    signals[PRINT_SIGNAL] =
        g_signal_new ("print_signal",
                      ...
                      NULL, NULL,
                      g_cclosure_user_marshal_VOID__POINTER_POINTER_UINT_UINT,
                      G_TYPE_NONE, 3, G_TYPE_STRV, G_TYPE_UINT, G_TYPE_UINT);
...
remote_object_connect (RemoteObject * obj,
                       ...
                       const char *name,
                       const char *desc,
                       const char *version, DBusGMethodInvocation * context)
{
    ...
    g_hash_table_insert (clients, sender, remote_object);
...
static gboolean
remote_object_disconnect (RemoteObject * obj, DBusGMethodInvocation * context)
{
    char *sender;
...
remote_object_set_context (RemoteObject * obj,
                           guint id, gboolean * ret, GError ** error)
{
    xchat_context *context;
...
remote_object_get_info (RemoteObject * obj,
                        const char *id, char **ret_info, GError ** error)
{
...
remote_object_get_prefs (RemoteObject * obj,
                         const char *name,
                         int *ret_type,
                         char **ret_str, int *ret_int, GError ** error)
{
    const char *str;
...
print_hook_cb (char *word[], void *userdata)
{
    ...
    g_signal_emit (info->obj,
                   signals[PRINT_SIGNAL],
                   0,
                   arg1, info->id, context_list_find_id (info->obj->context));
    g_strfreev (arg1);
...
remote_object_hook_command (RemoteObject * obj,
                            ...
                            int priority,
                            const char *help_text,
                            int return_value, guint * ret_id, GError ** error)
{
    HookInfo *info;
    ...
    info->hook = xchat_hook_command (ph,
                                     name,
                                     priority,
                                     command_hook_cb, help_text, info);
                           ...
                           int return_value, guint * ret_id, GError ** error)
{
    ...
    info->return_value = return_value;
    info->id = ++obj->last_hook_id;
    info->hook = xchat_hook_server (ph, name, priority, server_hook_cb, info);
    g_hash_table_insert (obj->hooks, &info->id, info);
    *ret_id = info->id;
                          ...
                          int return_value, guint * ret_id, GError ** error)
{
    ...
    info->return_value = return_value;
    info->id = ++obj->last_hook_id;
    info->hook = xchat_hook_print (ph, name, priority, print_hook_cb, info);
    g_hash_table_insert (obj->hooks, &info->id, info);
    *ret_id = info->id;
...
static gboolean
remote_object_unhook (RemoteObject * obj, guint id, GError ** error)
{
...
remote_object_list_get (RemoteObject * obj,
                        const char *name, guint * ret_id, GError ** error)
{
    xchat_list *xlist;
    guint *id;
...
remote_object_list_str (RemoteObject * obj,
                        guint id,
                        const char *name, char **ret_str, GError ** error)
{
    xchat_list *xlist;
...
remote_object_list_int (RemoteObject * obj,
                        guint id,
                        const char *name, int *ret_int, GError ** error)
{
    xchat_list *xlist;
...
remote_object_list_fields (RemoteObject * obj,
                           const char *name, char ***ret, GError ** error)
{
...
static gboolean
remote_object_list_free (RemoteObject * obj, guint id, GError ** error)
{
...
remote_object_emit_print (RemoteObject * obj,
                          const char *event_name,
                          const char *args[], gboolean * ret, GError ** error)
{
...
remote_object_nickcmp (RemoteObject * obj,
                       const char *nick1,
                       const char *nick2, int *ret, GError ** error)
{
...
remote_object_strip (RemoteObject * obj,
                     const char *str,
                     int len, int flag, char **ret_str, GError ** error)
{
...
static int
open_context_cb (char *word[], void *userdata)
{
    ContextInfo *info;
...
static int
close_context_cb (char *word[], void *userdata)
{
    GList *l;
