    ...
    switch (n)
      ...
      case 312:
          if (!serv->skip_next_whois)
              EMIT_SIGNAL (XP_TE_WHOIS3, whois_sess, word[4], word_eol[5],
                           NULL, NULL, 0);
          else
              inbound_user_info (sess, NULL, NULL, NULL, word[5], word[4],
                                 NULL, 0xff);
          break;
      ...
      case 321:
          if (!fe_is_chanwindow (sess->server))
              EMIT_SIGNAL (XP_TE_CHANLISTHEAD, serv->server_session, NULL,
                           NULL, NULL, NULL, 0);
          break;
      ...
      case 322:
          if (fe_is_chanwindow (sess->server))
            {
                fe_add_chan_list (sess->server, word[4], word[5],
                                  word_eol[6] + 1);
            }
          else
            {
      ...
      case 338:                /* Undernet Real user@host, Real IP */
          EMIT_SIGNAL (XP_TE_WHOIS_REALHOST, sess, word[4], word[5], word[6],
                       (word_eol[7][0] ==
                        ':') ? word_eol[7] + 1 : word_eol[7], 0);
          break;
#endif

      case 341:                /* INVITE ACK */
          EMIT_SIGNAL (XP_TE_UINVITE, sess, word[4], word[5],
              ...
              if (!strcmp (word[4], "152"))
                {
                    ...
                    inbound_user_info (sess, word[5], 0, 0, 0, word[6], 0,
                                       away);

                    /* try to show only user initiated whos */
                    if (!who_sess || !who_sess->doing_who)
                        EMIT_SIGNAL (XP_TE_SERVTEXT, serv->server_session,
                                     text, word[1], word[2], NULL, 0);
                }
              else
                  goto def;
          }
      ...
      case 348:                /* +e-list entry */
          if (!inbound_banlist
              (sess, atol (word[7]), word[4], word[5], word[6], TRUE))
              goto def;
          break;
      ...
      case 406:                /* WHOWAS error */
          EMIT_SIGNAL (XP_TE_SERVTEXT, whois_sess, text, word[1], word[2],
                       NULL, 0);
          serv->inside_whois = 0;
          break;
          ...
          if (is_channel (serv, word[4]))
            {
                ...
                EMIT_SIGNAL (XP_TE_SERVTEXT, realsess, text, word[1], word[2],
                             NULL, 0);
            }
          else
            {
...
static void
process_named_servermsg (session * sess, char *buf, char *rawname,
                         char *word_eol[])
{
    ...
    if (!strncmp (buf, "NOTICE ", 7))
      {
          ...
          if (*buf == ':')
              buf++;
          EMIT_SIGNAL (XP_TE_SERVNOTICE, sess, buf, sess->server->servername,
                       NULL, NULL, 0);
          return;
      }

    EMIT_SIGNAL (XP_TE_SERVTEXT, sess, buf, sess->server->servername, rawname,
                 NULL, 0);
}
