...
static int
cmd_allchannelslocal (session * sess, char *tbuf, char *word[],
                      char *word_eol[])
{
    GSList *list = sess_list;
          ...
          if (prefs.show_away_message)
            {
                ...
                sprintf (tbuf, "me is back (gone %.2d:%.2d:%.2d)",
                         gone / 3600, (gone / 60) % 60, gone % 60);
                for (list = sess_list; list; list = list->next)
                  {
                      /* am I the right server and not a dialog box */
                      if (((struct session *) list->data)->server ==
                          sess->server
                          && ((struct session *) list->data)->type ==
                          SESS_CHANNEL
                          && ((struct session *) list->data)->channel[0])
                        {
                            handle_command ((session *) list->data, tbuf,
                                            TRUE);
                        }
                  }
            }
          ...
          if (inet_addr (fullhost) != -1)       /* "fullhost" is really a IP number */
            {
                ...
                *lastdot = '.';
                ...
                switch (bantype)
                  {
                  case 0:
                      snprintf (tbuf, TBUFSIZE, "%s%s *!*@%s.*", mode, p2,
                                domain);
                      break;
                  ...
                  case 2:
                      snprintf (tbuf, TBUFSIZE, "%s%s *!%s@%s.*", mode, p2,
                                username, domain);
                      break;
                ...
                if (*word[3] && *word[4])
                  {
                      ...
                      if (!dcc)
                          EMIT_SIGNAL (XP_TE_NODCC, sess, NULL, NULL, NULL,
                                       NULL, 0);
...
menu_del (char *path, char *label)
{
    ...
    while (list)
      {
          me = list->data;
          if (!menu_streq (me->label, label, 1)
              && !menu_streq (me->path, path, 1))
            {
                menu_list = g_slist_remove (menu_list, me);
                fe_menu_del (me);
                menu_free (me);
...
menu_add (char *path, char *label, char *cmd, char *ucmd, int pos, int state,
          int markup, int enable, int mod, int key, char *group, char *icon)
{
    menu_entry *me;
    ...
    if (!strcasecmp (word[idx], "ADD"))
      {
          if (toggle)
            {
                menu_add (tbuf, label, word[idx + 2], word[idx + 3], pos,
                          state, markup, enable, mod, key, NULL, NULL);
            }
          else
            {
                if (word[idx + 2][0])
                    menu_add (tbuf, label, word[idx + 2], NULL, pos, state,
                              markup, enable, mod, key, group, icon);
                else
                    menu_add (tbuf, label, NULL, NULL, pos, state, markup,
                              enable, mod, key, group, icon);
            }
          return TRUE;
      }
    ...
    if (*nick)
      {
          if (strchr (nick, '.') == NULL)
            ...
            {
                snprintf (tbuf, TBUFSIZE, "exec -d %s %s", prefs.dnsprogram,
                          nick);
                                  ...
                                  if (backc)
                                    {
                                        ...
                                        j += sprintf (&nbuf[j], "\003%d,%02d",
                                                      colf, colb);
                                    }
                                  else
                                    {
                                        colf = escconv[colf % 14];
                                        j += sprintf (&nbuf[j], "\003%02d",
                                                      colf);
                                    }
                              }
                          cont:esc = FALSE;
    ...
    if (*cmd)
      {
          ...
          if (sess->running_exec != NULL)
            {
                EMIT_SIGNAL (XP_TE_ALREADYPROCESS, sess, NULL, NULL, NULL,
                             NULL, 0);
                return TRUE;
            }
          ...
          if (pid == -1)
          ...
          else
            {
                ...
                s->childpid = pid;
                s->iotag =
                    fe_input_add (s->myfd, FIA_READ | FIA_EX, exec_data, s);
                sess->running_exec = s;
                return TRUE;
            }
      }
          ...
          if (!*word[i])
            {
                ...
                mask = word[2];
                if (strchr (mask, '?') == NULL &&
                    strchr (mask, '*') == NULL && userlist_find (sess, mask))
                  {
                      mask = tbuf;
                ...
                switch (i)
                  {
                  case 1:
                      EMIT_SIGNAL (XP_TE_IGNOREADD, sess, mask, NULL, NULL,
                                   NULL, 0);
                      break;
                  case 2:      /* old ignore changed */
                      EMIT_SIGNAL (XP_TE_IGNORECHANGE, sess, mask, NULL, NULL,
                                   NULL, 0);
                  }
                return TRUE;
            }
...
static int
cmd_lagcheck (struct session *sess, char *tbuf, char *word[],
              char *word_eol[])
{
    lag_check ();
    return TRUE;
...
lastlog (session * sess, char *search, gboolean regexp)
{
    ...
    if (!lastlog_sess)
        lastlog_sess =
            new_ircwindow (sess->server, "(lastlog)", SESS_DIALOG, 0);
...
cmd_me (struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
    ...
    if (dcc_write_chat (sess->channel, tbuf))
      {
          /* print it to screen */
          inbound_action (sess, sess->channel, sess->server->nick, "", act,
                          TRUE, FALSE);
      }
    else
      {
                ...
                if (newsess)
                ...
                else
                  {
                      ...
                      if (strcasecmp (nick, "nickserv") == 0 &&
                          ...
                          msg = "identify ****";
                      EMIT_SIGNAL (XP_TE_MSGSEND, sess, nick, msg, NULL, NULL,
                                   0);
                  }
    ...
    if (*word[2] && *word_eol[3])
      {
          ...
          EMIT_SIGNAL (XP_TE_NOTICESEND, sess, word[2], word_eol[3], NULL,
                       NULL, 0);
          return TRUE;
      }
    return FALSE;
          ...
          while (1)
            {
                ...
                if (notify_deluser (word[i]))
                  {
                      EMIT_SIGNAL (XP_TE_DELNOTIFY, sess, word[i], NULL, NULL,
                                   NULL, 0);
                      return TRUE;
                  }
                ...
                if (net && strcmp (net, "ASK") == 0)
                ...
                else
                  {
                      ...
                      EMIT_SIGNAL (XP_TE_ADDNOTIFY, sess, word[i], NULL, NULL,
                                   NULL, 0);
                  }
            }
      }
               ...
               char *word_eol[])
{
    ...
    /* If it isn't "ALL" and there is something
    ...
    else if (*word[2])
      {
          ...
          if (*word[4 + offset])
              safe_strcpy (serv->password, word[4 + offset],
                           sizeof (serv->password));
          if (*word[3 + offset])
              serv->port = atoi (word[3 + offset]);
          safe_strcpy (serv->hostname, word[2 + offset],
                       sizeof (serv->hostname));
          serv->auto_reconnect (serv, TRUE, -1);
      }
    else
      {
...
static int
cmd_setcursor (struct session *sess, char *tbuf, char *word[],
               char *word_eol[])
{
    int delta = FALSE;
...
static int
parse_irc_url (char *url, char *server_name[], char *port[], char *channel[],
               int *use_ssl)
{
    char *co;
#ifdef USE_OPENSSL
...
cmd_server (struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
    ...
    if (!parse_irc_url
        (word[2 + offset], &server_name, &port, &channel, &use_ssl))
      {
          is_url = FALSE;
          ...
          if (ignore_del (mask, NULL))
            {
                if (strcasecmp (arg, "QUIET"))
                    EMIT_SIGNAL (XP_TE_IGNOREREMOVE, sess, mask, NULL, NULL,
                                 NULL, 0);
            }
          return TRUE;
      }
                ...
                if (sess->type == SESS_CHANNEL)
                  {
                      inbound_chanmsg (sess->server, NULL, sess->channel,
                                       ...
                                       FALSE);
                      sess->server->p_message (sess->server, sess->channel,
                                               word_eol[2]);
                  }
                list = list->next;
            }
...
const struct commands xc_cmds[] = {
    ...
    {"DNS", cmd_dns, 0, 0, 1,
     N_("DNS <nick|host|ip>, Finds a users IP number")},
...
#ifndef __EMX__
    {"EXECCONT", cmd_execc, 0, 0, 1,
     N_("EXECCONT, sends the process SIGCONT")},
#endif
    {"EXECKILL", cmd_execk, 0, 0, 1,
     N_
     ("EXECKILL [-9], kills a running exec in the current session. If -9 is given the process is SIGKILL'ed")},
#ifndef __EMX__
    {"EXECSTOP", cmd_execs, 0, 0, 1,
     N_("EXECSTOP, sends the process SIGSTOP")},
    {"EXECWRITE", cmd_execw, 0, 0, 1,
     N_("EXECWRITE, sends data to the processes stdin")},
#endif
#endif
    ...
    {"GUI", cmd_gui, 0, 0, 1,
     "GUI [APPLY|ATTACH|DETACH|SHOW|HIDE|FOCUS|FLASH|ICONIFY|COLOR <n>]\n"
    ...
    {"MENU", cmd_menu, 0, 0, 1,
     "MENU [-eX] [-i<ICONFILE>] [-k<mod>,<key>] [-m] [-pX] [-r<X,group>] [-tX] {ADD|DEL} <path> [command] [unselect command]\n"
    ...
    {"MSG", cmd_msg, 0, 0, 1,
     N_("MSG <nick> <message>, sends a private message")},
          ...
          if (src[0] == '%' || src[0] == '&')
            {
                if (isdigit ((unsigned char) src[1]))
                  {
                      if (isdigit ((unsigned char) src[2])
                      ...
                      switch (src[0])
                        ...
                        case 'y':
                            ...
                            snprintf (buf, sizeof (buf), "%4d%02d%02d", 1900 +
                                      tm_ptr->tm_year, 1 + tm_ptr->tm_mon,
                                      tm_ptr->tm_mday);
                            utf = buf;
                            break;
                        default:
                  ...
                  case '%':
                      ...
                      if (do_ascii &&
                           ...
                           && isdigit ((unsigned char) cmd[j + 3])))
                        {
                            ...
                            buf[i] = atoi (tbuf);
                            utf =
                                g_locale_to_utf8 (buf + i, 1, 0, &utf_len, 0);
                            if (utf)
                              {
    ...
    if (!rfc_ncasecmp (user->nick, data->nick, data->len))
      {
          ...
          if (lenu == data->len)
            {
                snprintf (data->tbuf, TBUFSIZE, "%s%s", user->nick,
                          data->space);
                data->len = -1;
                return FALSE;
            }
                ...
                if (len < NICKLEN)
                  {
                      ...
                      tree_foreach (sess->usertree,
                                    (tree_traverse_func *) nick_comp_cb,
                                    &data);
                      ...
                      if (data.best)
                        {
                            snprintf (tbuf, TBUFSIZE, "%s%s", data.best->nick,
                                      space - 1);
                            return;
                        }
                  }
    ...
    if (sess->server->connected)
      {
          ...
          inbound_chanmsg (sess->server, sess, sess->channel,
                           sess->server->nick, text, TRUE, FALSE);
...
handle_command (session * sess, char *cmd, int check_spch)
{
    ...
    if (command_level > 99)
      {
          fe_message (_("Too many recursive usercommands, aborting."),
                      FE_MSG_ERROR);
          return TRUE;
      }
    command_level++;
