...
typedef int (xchat_init_func) (xchat_plugin *, char **, char **, char **,
                               char *);
    ...
    if (!fake)
      {
          ...
          if (((xchat_init_func *) init_func) (pl, &pl->name, &pl->desc,
                                               &pl->version, arg) == 0)
            {
    ...
    while (list)
      {
          ...
          if ((by_filename && pl->filename
               && strcasecmp (name, pl->filename) == 0) || (by_filename
                                                            && pl->filename
                                                            &&
                                                            strcasecmp (name,
                                                                        file_part
...
plugin_load (session * sess, char *filename, char *arg)
{
    ...
    if (!g_module_symbol
        (handle, "xchat_plugin_init", (gpointer *) & init_func))
      {
    ...
    if (!g_module_symbol
        (handle, "xchat_plugin_deinit", (gpointer *) & deinit_func))
        deinit_func = NULL;
...
#define RTLD_NOW 0
    ...
    if (filepart &&
        ...
        (strstr (filepart, "local")
         || strncmp (filepart, "libxsys-1", 9) == 0))
    ...
    while (1)
      {
          ...
          switch (type)
            ...
            case HOOK_COMMAND:
                ret =
                    ((xchat_cmd_cb *) hook->callback) (word, word_eol,
                                                       hook->userdata);
                break;
            case HOOK_SERVER:
                ret =
                    ((xchat_serv_cb *) hook->callback) (word, word_eol,
                                                        hook->userdata);
                break;
            default:           /*case HOOK_PRINT: */
                ret =
                    ((xchat_print_cb *) hook->callback) (word,
                                                         hook->userdata);
                break;
            }
...
int
plugin_emit_command (session * sess, char *name, char *word[],
                     char *word_eol[])
{
...
int
plugin_emit_server (session * sess, char *name, char *word[],
                    char *word_eol[])
{
...
{
    int flags = 0, ret;
    typedef int (xchat_fd_cb2) (int fd, int flags, void *user_data,
                                GIOChannel *);
    ...
    ret =
        ((xchat_fd_cb2 *) hook->callback) (hook->pri, flags, hook->userdata,
                                           source);
...
plugin_add_hook (xchat_plugin * pl, int type, int pri, const char *name,
                 const char *help_text, void *callb, int timeout,
                 void *userdata)
{
    xchat_hook *hook;
...
xchat_hook_command (xchat_plugin * ph, const char *name, int pri,
                    xchat_cmd_cb * callb, const char *help_text,
                    void *userdata)
{
...
{
    return plugin_add_hook (ph, HOOK_SERVER, pri, name, 0, callb, 0,
                            userdata);
}
...
xchat_hook_timer (xchat_plugin * ph, int timeout, xchat_timer_cb * callb,
                  void *userdata)
{
    return plugin_add_hook (ph, HOOK_TIMER, 0, 0, 0, callb, timeout,
                            userdata);
}
...
xchat_print (xchat_plugin * ph, const char *text)
{
    if (!is_session (ph->context))
      {
          DEBUG (PrintTextf
                 (0, "%s\txchat_print called without a valid context.\n",
                  ph->name));
          return;
      }
...
xchat_command (xchat_plugin * ph, const char *command)
{
    ...
    if (!is_session (ph->context))
      {
          DEBUG (PrintTextf
                 (0, "%s\txchat_command called without a valid context.\n",
                  ph->name));
          return;
      }
...
xchat_context *
xchat_find_context (xchat_plugin * ph, const char *servname,
                    const char *channel)
{
...
xchat_get_info (xchat_plugin * ph, const char *id)
{
    ...
    if (!strncmp (id, "event_text", 10))
      {
          char *e = (char *) id + 10;
          if (*e == ' ')
              e++;              /* 2.8.0 only worked without a space */
    ...
    if (!is_session (sess))
      {
          DEBUG (PrintTextf
                 (0, "%s\txchat_get_info called without a valid context.\n",
                  ph->name));
          return NULL;
      }
...
int
xchat_get_prefs (xchat_plugin * ph, const char *name, const char **string,
                 int *integer)
{
    int i = 0;
...
xchat_list_fields (xchat_plugin * ph, const char *name)
{
    ...
    static const char *const channels_fields[] = {
        "schannel", "schantypes", "pcontext", "iflags", "iid", "ilag",
            "imaxmodes",
        "snetwork", "snickmodes", "snickprefixes", "iqueue", "sserver",
            "itype", "iusers",
        NULL
    };
    ...
    static const char *const users_fields[] = {
        "iaway", "shost", "tlasttalk", "snick", "sprefix", "srealname",
            "iselected", NULL
    };
...
{
#ifdef USE_PLUGIN
    ph = plugin_list_add (NULL, strdup (filename), strdup (name),
                          strdup (desc), strdup (version), NULL, NULL, TRUE,
                          TRUE);
    fe_pluginlist_update ();
#endif
                  ...
                  int modes_per_line, char sign, char mode)
{
    ...
    send_channel_modes (ph->context, tbuf, (char **) targets, 0, ntargets,
                        sign, mode, modes_per_line);
}
