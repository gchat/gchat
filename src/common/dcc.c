...
static double
timeval_diff (GTimeVal * greater, GTimeVal * less)
{
    long usecdiff;
    double result;
                      ...
                      if (prefs.dccstalltimeout > 0)
                        {
                            if (!dcc->throttled
                                && tim - dcc->lasttime >
                                prefs.dccstalltimeout)
                              {
                                  EMIT_SIGNAL (XP_TE_DCCSTALL,
                                               dcc->serv->front_session,
                                               dcctypes[dcc->type],
                                               file_part (dcc->file),
                                               dcc->nick, NULL, 0);
          ...
          switch (dcc->dccstat)
            ...
            case STAT_ACTIVE:
                ...
                switch (dcc->type)
                  ...
                  case TYPE_CHATSEND:
                  case TYPE_CHATRECV:
                      EMIT_SIGNAL (XP_TE_DCCCHATABORT, sess, dcc->nick, NULL,
                                   NULL, NULL, 0);
                      break;
                  case TYPE_SEND:
...
dcc_write_chat (char *nick, char *text)
{
    ...
    if (dcc && dcc->dccstat == STAT_ACTIVE)
      {
          len = strlen (text);
          tcp_send_real (NULL, dcc->sok, dcc->serv->encoding,
                         dcc->serv->using_irc, text, len);
          send (dcc->sok, "\n", 1, 0);
          dcc->size += len;
          fe_dcc_update (dcc);
          return dcc;
...
dcc_chat_line (struct DCC *dcc, char *line)
{
    ...
    if (dcc->serv->using_irc)   /* using "IRC" encoding (CP1252/UTF-8 hybrid) */
        ...
        utf =
            g_convert (line, len, "UTF-8", dcc->serv->encoding, 0, &utf_len,
                       0);
    ...
    if (line[0] == 1 && !strncasecmp (line + 1, "ACTION", 6))
      {
          ...
          if (po)
              po[0] = 0;
          inbound_action (sess, dcc->serv->nick, dcc->nick, "", line + 8,
                          FALSE, FALSE);
      }
    else
      {
    ...
    while (1)
      {
          ...
          if (!dcc->iotag)
              dcc->iotag =
                  fe_input_add (dcc->sok, FIA_READ | FIA_EX, dcc_read_chat,
                                dcc);
          ...
          if (len < 1)
            {
                ...
                sprintf (portbuf, "%d", dcc->port);
                EMIT_SIGNAL (XP_TE_DCCCHATF, dcc->serv->front_session,
                             dcc->nick, net_ip (dcc->addr), portbuf,
    ...
    if (dcc->fp == -1)
      {
          ...
          if (dcc->resumable)
            {
                dcc->fp =
                    open (dcc->destfile_fs, O_WRONLY | O_APPEND | OFLAGS);
                ...
                if (access (dcc->destfile_fs, F_OK) == 0)
                  {
                      ...
                      old = dcc->destfile;
                      dcc->destfile =
                          xchat_filename_to_utf8 (buf, -1, 0, 0, 0);
    ...
    if (dcc->fp == -1)
      {
          ...
          EMIT_SIGNAL (XP_TE_DCCFILEERR, dcc->serv->front_session,
                       dcc->destfile, errorstring (errno), NULL, NULL, 0);
          dcc_close (dcc, STAT_FAILED, FALSE);
          return TRUE;
      }
    while (1)
      {
          ...
          if (!dcc->iotag)
              dcc->iotag =
                  fe_input_add (dcc->sok, FIA_READ | FIA_EX, dcc_read, dcc);
          ...
          if (n < 1)
            {
                ...
                EMIT_SIGNAL (XP_TE_DCCRECVERR, dcc->serv->front_session,
...
static gboolean
dcc_connect_finished (GIOChannel * source, GIOCondition condition,
                      struct DCC *dcc)
{
    char host[128];
    ...
    switch (dcc->type)
      ...
      case TYPE_RECV:
          dcc->iotag =
              fe_input_add (dcc->sok, FIA_READ | FIA_EX, dcc_read, dcc);
          EMIT_SIGNAL (XP_TE_DCCCONRECV, dcc->serv->front_session, dcc->nick,
                       host, dcc->file, NULL, 0);
          break;
      case TYPE_SEND:
          /* passive send */
          ...
          if (dcc->fastsend)
              dcc->wiotag =
                  fe_input_add (dcc->sok, FIA_WRITE, dcc_send_data, dcc);
          dcc->iotag =
              fe_input_add (dcc->sok, FIA_READ | FIA_EX, dcc_read_ack, dcc);
...
static gboolean
dcc_wingate_proxy_traverse (GIOChannel * source, GIOCondition condition,
                            struct DCC *dcc)
{
    struct proxy_state *proxy = dcc->proxy;
    if (proxy->phase == 0)
      {
          proxy->buffersize =
              snprintf ((char *) proxy->buffer, MAX_PROXY_BUFFER, "%s %d\r\n",
...
static gboolean
dcc_socks_proxy_traverse (GIOChannel * source, GIOCondition condition,
                          struct DCC *dcc)
{
    struct proxy_state *proxy = dcc->proxy;
...
static gboolean
dcc_socks5_proxy_traverse (GIOChannel * source, GIOCondition condition,
                           struct DCC *dcc)
{
    struct proxy_state *proxy = dcc->proxy;
...
static gboolean
dcc_http_proxy_traverse (GIOChannel * source, GIOCondition condition,
                         struct DCC *dcc)
{
    struct proxy_state *proxy = dcc->proxy;
...
static gboolean
dcc_proxy_connect (GIOChannel * source, GIOCondition condition,
                   struct DCC *dcc)
{
    fe_input_remove (dcc->iotag);
    dcc->iotag = 0;
...
dcc_connect (struct DCC *dcc)
{
    ...
    if (dcc->pasvid && dcc->port == 0)
    ...
    else
      {
          ...
          if (DCC_USE_PROXY ())
              dcc->iotag =
                  fe_input_add (dcc->sok, FIA_WRITE | FIA_EX,
                                dcc_proxy_connect, dcc);
          else
              dcc->iotag =
                  fe_input_add (dcc->sok, FIA_WRITE | FIA_EX,
                                dcc_connect_finished, dcc);
      }
          ...
          if (len < 1)
            {
                ...
                EMIT_SIGNAL (XP_TE_DCCSENDFAIL, dcc->serv->front_session,
                             file_part (dcc->file), dcc->nick,
                             errorstring ((len < 0) ? sock_error () : 0),
                             NULL, 0);
                dcc_close (dcc, STAT_FAILED, FALSE);
                return TRUE;
            }
    ...
    switch (dcc->type)
      ...
      case TYPE_SEND:
          if (dcc->fastsend)
              dcc->wiotag = fe_input_add (sok, FIA_WRITE, dcc_send_data, dcc);
          dcc->iotag =
              fe_input_add (sok, FIA_READ | FIA_EX, dcc_read_ack, dcc);
          dcc_send_data (NULL, 0, (gpointer) dcc);
          EMIT_SIGNAL (XP_TE_DCCCONSEND, dcc->serv->front_session,
                       dcc->nick, host, dcc->file, NULL, 0);
          break;
...
dcc_get_my_address (void)       /* the address we'll tell the other person */
{
    ...
    if (prefs.ip_from_server && prefs.dcc_ip)
    ...
    else if (prefs.dcc_ip_str[0])
      {
          ...
          if (dns_query != NULL &&
              dns_query->h_length == 4 && dns_query->h_addr_list[0] != NULL)
            {
    ...
    if (prefs.first_dcc_send_port > 0)
      {
          ...
          while ((prefs.last_dcc_send_port > ntohs (SAddr.sin_port)) &&
                 (bindretval == -1))
            {
                ...
                bindretval =
                    bind (dcc->sok, (struct sockaddr *) &SAddr,
                          sizeof (SAddr));
            }

          /* with a small port range, reUseAddr is needed */
          len = 1;
          setsockopt (dcc->sok, SOL_SOCKET, SO_REUSEADDR, (char *) &len,
                      sizeof (len));
                ...
                if (passive || dcc_listen_init (dcc, sess))
                  {
                      ...
                      EMIT_SIGNAL (XP_TE_DCCOFFER, sess,
                                   file_part (dcc->file), to, dcc->file, NULL,
                                   0);
                  }
                else
                  {
...
#warning no win32 implementation - behaviour may be unreliable
    ...
    if ((dcc->fp ==
         -1 ? stat (dcc->destfile_fs, &st_a) : fstat (dcc->fp, &st_a)) == -1)
        return FALSE;
    ...
    switch (dcc->dccstat)
      ...
      case STAT_QUEUED:
          if (dcc->type != TYPE_CHATSEND)
            {
                if (dcc->type == TYPE_RECV && prefs.autoresume
                    && dcc->resumable)
                  {
                      dcc_resume (dcc);
                  }
    ...
    if (dcc)
      {
          switch (dcc->dccstat)
            ...
            case STAT_QUEUED:
            case STAT_CONNECTING:
                EMIT_SIGNAL (XP_TE_DCCCHATREOFFER, sess, nick, NULL, NULL,
                             NULL, 0);
                return;
            case STAT_ABORTED:
            case STAT_FAILED:
    ...
    if (passive || dcc_listen_init (dcc, sess))
      {
          ...
          EMIT_SIGNAL (XP_TE_DCCCHATOFFERING, sess, nick, NULL, NULL, NULL,
                       0);
      }
    else
      {
          ...
          if (prefs.autodccchat == 1)
          ...
          else if (prefs.autodccchat == 2)
            {
                ...
                snprintf (buff, sizeof (buff),
                          "%s is offering DCC Chat. Do you want to accept?",
                          nick);
...
static struct DCC *
dcc_add_file (session * sess, char *file, DCC_SIZE size, int port, char *nick,
    ...
    if (dcc)
      {
          ...
          /* get the local filesystem encoding */
          dcc->destfile_fs =
              xchat_filename_from_utf8 (dcc->destfile, -1, 0, 0, 0);
          ...
          if (prefs.autodccsend == 1)
          ...
          else if (prefs.autodccsend == 2)
            {
                snprintf (tbuf, sizeof (tbuf),
                          _("%s is offering \"%s\". Do you want to accept?"),
                          nick, file);
                      ...
                      if (dcc->pasvid)
                          snprintf (tbuf, sizeof (tbuf),
                                    strchr (file_part (dcc->file),
                                            ' ') ? "DCC ACCEPT \"%s\" %d %"
                                    DCC_SFMT " %d" : "DCC ACCEPT %s %d %"
                                    DCC_SFMT " %d", file_part (dcc->file),
                                    port, dcc->resumable, dcc->pasvid);
                      else
                          snprintf (tbuf, sizeof (tbuf),
                                    strchr (file_part (dcc->file),
                                            ' ') ? "DCC ACCEPT \"%s\" %d %"
                                    DCC_SFMT : "DCC ACCEPT %s %d %" DCC_SFMT,
    ...
    while (list)
      {
          ...
          i++;
          PrintTextf (sess,
                      " %s  %-10.10s %-7.7s %-7" DCC_SFMT " %-7" DCC_SFMT
