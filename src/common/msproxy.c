...
int
traverse_msproxy (int sok, char *serverAddr, int port,
                  struct msproxy_state_t *state, netstore * ns_proxy,
                  int csok4, int csok6, int *csok, char bound)
{
    ...
    memcpy (ntdomain, &res.packet.auth.NTLMSSP[res.packet.auth.target.offset],
            res.packet.auth.target.len);
    ...
    req.packet.auth2.clienthost_buf.alloc =
        req.packet.auth2.clienthost_buf.len;
