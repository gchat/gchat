...
scrollback_save (session * sess, char *text)
{
    ...
    if (sizeof (stamp) == 4)    /* gcc will optimize one of these out */
        write (sess->scrollfd, buf,
               snprintf (buf, sizeof (buf), "T %d ", (int) stamp));
    else
        write (sess->scrollfd, buf,
               snprintf (buf, sizeof (buf), "T %" G_GINT64_FORMAT " ",
                         (gint64) stamp));
...
log_create_pathname (char *servname, char *channame, char *netname)
{
    ...
    log_insert_vars (fname, sizeof (fname), prefs.logmask, channame, netname,
                     servname);
    free (channame);
    ...
    if (fnametime[0] == '/')    /* is it fullpath already? */
        ...
        snprintf (fname, sizeof (fname), "%s/xchatlogs/%s", get_xdir_utf8 (),
                  fnametime);
...
int
text_emit_by_name (char *name, session * sess, char *a, char *b, char *c,
                   char *d)
{
    int i = 0;
