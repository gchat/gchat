                ...
                int fromme, int id)
{
    ...
    if (!fromme && !privaction)
    {
        ...
        {
            EMIT_SIGNAL (XP_TE_HCHANACTION, sess, from, text, nickchar,
                         idtext, 0);
            return;
        }
    }
    ...
    while (list)
    {
        ...
        if (sess->server == serv)
        {
                    ...
                    || (me && sess->type == SESS_SERVER))
            {
                ...
                {
                    if (me)
                        EMIT_SIGNAL (XP_TE_UCHANGENICK, sess, nick,
                                     newnick, NULL, NULL, 0);
                    else
                        EMIT_SIGNAL (XP_TE_CHANGENICK, sess, nick,
                                     newnick, NULL, NULL, 0);
                }
            }
            if (sess->type == SESS_DIALOG
...
inbound_ukick (server * serv, char *chan, char *kicker, char *reason)
{
    ...
    if (sess)
    {
        EMIT_SIGNAL (XP_TE_UKICK, sess, serv->nick, chan, kicker, reason,
                     0);
        clear_channel (sess);
        if (prefs.autorejoin)
        {
...
inbound_upart (server * serv, char *chan, char *ip, char *reason)
{
    ...
    {
        if (*reason)
            EMIT_SIGNAL (XP_TE_UPARTREASON, sess, serv->nick, ip, chan,
                         reason, 0);
        else
...
inbound_nameslist (server * serv, char *chan, char *names)
{
    ...
    if (!sess)
    {
        EMIT_SIGNAL (XP_TE_USERSONCHAN, serv->server_session, chan, names,
                     NULL, NULL, 0);
        return;
    }
...
inbound_quit (server * serv, char *nick, char *ip, char *reason)
{
    ...
    while (list)
    {
        ...
        if (sess->server == serv)
        {
            ...
            {
                EMIT_SIGNAL (XP_TE_QUIT, sess, nick, reason, ip, NULL,
                             0);
            }
            else if (sess->type == SESS_DIALOG
                     && !serv->p_cmp (sess->channel, nick))
...
inbound_next_nick (session * sess, char *nick)
{
    ...
    case 3:
        ...
        EMIT_SIGNAL (XP_TE_NICKCLASH, sess, nick, prefs.nick3, NULL, NULL,
                     0);
        break;
