    ...
    if (a_len < b_len)
    ...
    else if (b_len == 1)
      {
          ...
          q_ptr = roomptr + 1;
          {
              ...
              for (count = a_len; count > 0; count--)
                {
                    mp_twolimb_t num =
                        ((mp_twolimb_t) remainder << GMP_LIMB_BITS) |
                        *--sourceptr;
                    *--destptr = num / den;
                    remainder = num % den;
                }
    ...
    else
      {
          ...
          if (s > 0)
            {
                tmp_roomptr =
                    (mp_limb_t *) malloc (b_len * sizeof (mp_limb_t));
                if (tmp_roomptr == NULL)
                  {
                      free (roomptr);
                      return NULL;
              ...
              for (;;)
                {
                    ...
                    if (r_ptr[j + b_len] < b_msd)       /* r[j+n] < b[n-1] ? */
                      {
                          ...
                          mp_twolimb_t num =
                              ((mp_twolimb_t) r_ptr[j + b_len] <<
                               GMP_LIMB_BITS) | r_ptr[j + b_len - 1];
                          q_star = num / b_msd;
                          c1 = num % b_msd;
                      }
                    else
                       ...
                       c1 = (r[j+n]*beta+r[j+n-1]) - q* * b[n-1] (>=0, <beta).  */
                    {
                        mp_twolimb_t c2 =       /* c1*beta+r[j+n-2] */
                            ((mp_twolimb_t) c1 << GMP_LIMB_BITS) | r_ptr[j +
                                                                         b_len
                                                                         - 2];
                              ...
                              for (count = b_len; count > 0; count--)
                                {
                                    ...
                                    carry =
                                        carry
                                        +
                                        (mp_twolimb_t) q_star *(mp_twolimb_t)
                                        * sourceptr++ +
                                        (mp_limb_t) ~ (*destptr);
                                    ...
                                    for (count = b_len; count > 0; count--)
                                      {
                                          ...
                                          *destptr++ =
                                              source1 + source2 + carry;
                                          carry =
                                              (carry ? source1 >=
                                               (mp_limb_t) ~ source2 : source1
                                               > (mp_limb_t) ~ source2);
                                      }
                                }
        ...
        for (i = b_len;;)
          {
              mp_limb_t r_i =
                  (i <= r_len
                   && i > 0 ? r_ptr[i - 1] >> (GMP_LIMB_BITS - 1) : 0) | (i <
                                                                          r_len
                                                                          ?
                                                                          r_ptr
                                                                          [i]
...
decode_double (double x, int *ep, mpn_t * mp)
{
...
#if (DBL_MANT_BIT % GMP_LIMB_BITS) > GMP_LIMB_BITS / 2
    {
        ...
        if (!(y >= 0.0 && y < 1.0))
            abort ();
        m.limbs[DBL_MANT_BIT / GMP_LIMB_BITS] =
            (hi << (GMP_LIMB_BITS / 2)) | lo;
    }
#else
    {
    ...
    if (abs_n > 0)
      {
          ...
          for (n13 = 0; n13 <= abs_n; n13 += 13)
            {
                mp_limb_t digit1 =
                    small_pow5[n13 + 13 <= abs_n ? 13 : abs_n - n13];
                size_t j;
                mp_twolimb_t carry = 0;
...
floorlog10l (long double x)
{
    ...
    if (y < 0.5L)
      {
          while (y <
                 (1.0L / (1 << (GMP_LIMB_BITS / 2)) /
                  (1 << (GMP_LIMB_BITS / 2))))
            {
                y *= 1.0L * (1 << (GMP_LIMB_BITS / 2)) *
                    (1 << (GMP_LIMB_BITS / 2));
                exp -= GMP_LIMB_BITS;
            }
          if (y < (1.0L / (1 << 16)))
    ...
    l -= 1.4426950408889634074 * z * (1.0 +
                                      z * (0.5 + z * ((1.0 / 3) + z * 0.25)));
...
floorlog10 (double x)
{
    ...
    if (y < 0.5)
      {
          while (y <
                 (1.0 / (1 << (GMP_LIMB_BITS / 2)) /
                  (1 << (GMP_LIMB_BITS / 2))))
            {
                y *= 1.0 * (1 << (GMP_LIMB_BITS / 2)) *
                    (1 << (GMP_LIMB_BITS / 2));
                exp -= GMP_LIMB_BITS;
            }
          if (y < (1.0 / (1 << 16)))
  ...
  if (a.arg)                                                            \
...

    {
        ...
        if (buf_neededlength < 4000 / sizeof (TCHAR_T))
          ...
          {
              size_t buf_memsize =
                  xtimes (buf_neededlength, sizeof (TCHAR_T));
              ...
              if (cp != dp->dir_start)
                {
                    ...
                    if (sizeof (FCHAR_T) == sizeof (DCHAR_T))
                      {
                          DCHAR_CPY (result + length, (const DCHAR_T *) cp,
                                     n);
                          ...
                          switch (a.arg[dp->arg_index].type)
                            ...
                            case TYPE_COUNT_SCHAR_POINTER:
                                *a.arg[dp->arg_index].a.
                                    a_count_schar_pointer = length;
                                break;
                            case TYPE_COUNT_SHORT_POINTER:
                                *a.arg[dp->arg_index].a.
                                    a_count_short_pointer = length;
                                break;
                            case TYPE_COUNT_INT_POINTER:
                                *a.arg[dp->arg_index].a.a_count_int_pointer =
                                    length;
                                break;
                            case TYPE_COUNT_LONGINT_POINTER:
                                *a.arg[dp->arg_index].a.
                                    a_count_longint_pointer = length;
                                break;
#if HAVE_LONG_LONG_INT
                            case TYPE_COUNT_LONGLONGINT_POINTER:
                                *a.arg[dp->arg_index].a.
                                    a_count_longlongint_pointer = length;
                                break;
#endif
                            default:
                                abort ();
                    ...
                    /* The unistdio extensions.  */
                    else if (dp->conversion == 'U')
                      {
                          ...
                          if (dp->precision_start != dp->precision_end)
                            {
                                if (dp->precision_arg_index != ARG_NONE)
                                  ...
                                  {
                                      const FCHAR_T *digitp =
                                          dp->precision_start + 1;

                                      precision = 0;
                                      while (digitp != dp->precision_end)
                                          precision =
                                              xsum (xtimes (precision, 10),
                                                    *digitp++ - '0');
                                      has_precision = 1;
                                  }
                            }
                          ...
                          switch (type)
                            ...
                            case TYPE_U8_STRING:
                                {
                                    const uint8_t *arg =
                                        a.arg[dp->arg_index].a.a_u8_string;
                                    ...
                                    {   /* Convert.  */
                                        ...
                                        converted =
                                            u8_conv_to_encoding
                                            (locale_charset (),
                                             iconveh_question_mark, arg,
                                             arg_end - arg, NULL, converted,
                                             &converted_len);
#else
                        ...
                        )
                      {
...

                          {
                              ...
                              /* Append the result.  */
                              memcpy (result + length, tmp,
                                      count * sizeof (DCHAR_T));
                              if (tmp != tmpbuf)
                                  free (tmp);
                              length += count;
                          }
                             ...
                             ))
                      {
                          ...
                          tmp_length =
                              (type ==
                               TYPE_LONGDOUBLE ? LDBL_DIG + 1 : DBL_DIG + 1);
#elif NEED_PRINTF_INFINITE_DOUBLE && NEED_PRINTF_LONG_DOUBLE
                          tmp_length =
                              (type == TYPE_LONGDOUBLE ? LDBL_DIG + 1 : 0);
                                ...
                                if (dp->conversion == 'c'
                                    || dp->conversion == 's')
                                  {
                                      ...
                                      tmpdst =
                                          ...
                                          (locale_charset (),
                                           iconveh_question_mark, tmpsrc,
                                           count, NULL, NULL, &tmpdst_len);
                                      if (tmpdst == NULL)
                                        {
                                            int saved_errno = errno;
                                            if (!
                                      ...
                                      ENSURE_ALLOCATION (xsum
                                                         (length,
                                                          tmpdst_len));
                                      DCHAR_CPY (result + length, tmpdst,
                                                 tmpdst_len);
                                      free (tmpdst);
                                      count = tmpdst_len;
                                  }
        ...
        if (result != resultbuf && length + 1 < allocated)
          {
              ...
              memory =
                  (DCHAR_T *) realloc (result,
                                       (length + 1) * sizeof (DCHAR_T));
              if (memory != NULL)
                  result = memory;
          }
