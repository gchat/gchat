...
#define CATEGORYNAME_INITIALIZED
    ...
    if (foundp != NULL && (*foundp)->counter == _nl_msg_cat_cntr)
      {
          ...
          if (plural)
              retval =
                  plural_lookup ((*foundp)->domain, n, (*foundp)->translation,
          ...
          if (!IS_ABSOLUTE_PATH (dirname))
            {
                ...
                for (;;)
                  {
                      resolved_dirname =
                          (char *) alloca (path_max + dirname_len);
                ...
                stpcpy (stpcpy (strchr (resolved_dirname, '\0'), "/"),
                        dirname);
    ...
    stpcpy ((char *)
            mempcpy (stpcpy (stpcpy (xdomainname, categoryname), "/"),
    ...
    while (1)
      {
          ...
          domain =
              _nl_find_domain (dirname, single_locale, xdomainname, binding);

          if (domain != NULL)
            {
#if defined IN_LIBGLOCALE
                retval =
                    _nl_find_msg (domain, binding, encoding, msgid1, &retlen);
#else
...
#if defined IN_LIBGLOCALE
                            retval =
                                _nl_find_msg (domain->successor[cnt], binding,
...
#endif
                            newp =
                                (struct known_translation_t *) malloc (size);
                            if (newp != NULL)
                              {
                                  char *new_domainname;
#ifdef HAVE_PER_THREAD_LOCALE
                                  ...
                                  new_domainname =
                                      (char *) mempcpy (newp->msgid.appended,
                                                        msgid1, msgid_len);
                                  memcpy (new_domainname, domainname,
                                          domainname_len + 1);
#ifdef HAVE_PER_THREAD_LOCALE
                                  new_localename =
                                      new_domainname + domainname_len + 1;
          ...
          while (1)
            {
                ...
                if (nstr < nstrings
                    ...
                    (strcmp
                     ...
                     0) : domain->orig_sysdep_tab[nstr - nstrings].length >
                    len
                    &&
                    (strcmp
                     (msgid,
                      domain->orig_sysdep_tab[nstr - nstrings].pointer) == 0))
                  {
                      act = nstr;
                      goto found;
                  }
    ...
    if (act < nstrings)
      {
          ...
          resultlen =
              W (domain->must_swap, domain->trans_tab[act].length) + 1;
      }
    else
      {
          ...
          if (convd == NULL)
            {
                ...
                for (i = nconversions; i > 0;)
                  {
                      i--;
                      if (strcmp (domain->conversions[i].encoding, encoding)
                          == 0)
                        {
                            convd = &domain->conversions[i];
                            goto found_convd;
                              ...
                              if (charsetstr != NULL)
                                {
                                    ...
                                    *((char *)
                                      mempcpy (charset, charsetstr, len)) =
                               '\0';
#else
                                    ...
                                    int r =
                                        __gconv_open (outcharset, charset,
                                                      &convd->conv,
                                                      GCONV_AVOID_NOCONV);
                                    ...
                                    if (strchr (outcharset, '/') == NULL)
                                      {
                                          ...
                                          tmp =
                                              (char *) alloca (len + 10 + 1);
                                          memcpy (tmp, outcharset, len);
                                          memcpy (tmp + len, "//TRANSLIT",
                                                  10 + 1);
                                          outcharset = tmp;
                            ...
                            if (malloc_count > 0)
                              {
                                  ++malloc_count;
                                  freemem_size =
                                      malloc_count * INITIAL_BLOCK_SIZE;
                                  newmem =
                                      (transmem_block_t *)
                                      realloc (transmem_list, freemem_size);
#ifdef _LIBC
                                  if (newmem != NULL)
