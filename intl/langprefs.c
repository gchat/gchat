...
_nl_language_preferences_win32_mui (HMODULE kernel32)
{
    ...
    typedef DWORD (WINAPI * GetUserPreferredUILanguages_func) (ULONG, PULONG,
                                                               PWSTR, PULONG);
    ...
    if (p_GetUserPreferredUILanguages != NULL)
      {
          ...
          ret = p_GetUserPreferredUILanguages (MUI_LANGUAGE_NAME,
                                               ...
                                               NULL, &bufsize);
          if (ret == 0
              && GetLastError () == STATUS_BUFFER_OVERFLOW && bufsize > 0)
            {
...
_nl_language_preferences_win32_ME (HMODULE kernel32)
{
    ...
    if (p_GetUserDefaultUILanguage != NULL)
        return
            _nl_locale_name_from_win32_LANGID (p_GetUserDefaultUILanguage ());
    return NULL;
}
...
ret_first_language (HMODULE h, LPCSTR type, LPCSTR name, WORD lang,
                    LONG_PTR param)
{
        ...
        if (!cache_initialized && getenv ("GETTEXT_MUI") != NULL)
          {
              ...
              if (languages == NULL && kernel32 != NULL)
                  languages =
                      _nl_language_preferences_win32_system (kernel32);
